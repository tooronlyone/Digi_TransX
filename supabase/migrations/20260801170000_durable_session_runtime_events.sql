-- Phase 1B-2C3A: activate only durable-session issuance and current-session revocation.
-- Runtime persistence continues to use the existing user_sessions foundation.
...
declare
    ...
    expected_pre constant text := '11043982605bef207d3b9a5626bd86d8';
    expected_post constant text := '87c1377e1404933c69b1a90ac9962937';
begin
    ...
    if actual_signature = expected_post then
        if (select count(*) from public.canonical_event_catalog_projection) <> 170
           ...
           or (select count(*) from public.canonical_event_catalog_projection where event_name in ('security.session.issued','security.session.revoked') and integrated and writable and lifecycle_status='planned') <> 2 then
            raise exception using errcode='55000', message='durable-session runtime event post-state is partial or incompatible';
        end if;
        return;
    end if;
    ...
    if actual_signature is distinct from expected_pre
       ...
       or (select count(*) from public.canonical_event_catalog_projection where integrated) <> 10
       or (select count(*) from public.canonical_event_catalog_projection where event_name in ('security.session.issued','security.session.revoked') and not integrated and writable and lifecycle_status='planned') <> 2 then
        raise exception using errcode='55000', message='durable-session runtime event pre-state is partial or incompatible';
    end if;
    ...
    if pg_temp.canonical_event_semantic_signature() is distinct from expected_post
       or (select count(*) from public.canonical_event_catalog_projection where integrated) <> 12 then
        raise exception using errcode='55000', message='durable-session runtime event activation failed';
    end if;
end
$migration$;
