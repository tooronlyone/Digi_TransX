-- Phase 1B-2A correction: reject writable catalog definitions until integrated.
-- Forward-only hardening of the existing canonical INSERT guard; no event rows,
-- catalog rows, triggers, ACLs, policies, or unrelated objects are changed.
...
declare
    ...
    expected_pre_signature constant text := 'f5168975e0605fe0f7b84c1276a0082a';
    expected_post_signature constant text := '7b8157021244549cfed79416b40ab662';
...
begin
    ...
    if integrated_names is distinct from expected_integrated_names
       ...
       or (select count(*) from public.canonical_event_catalog_projection where integrated) <> 4 then
        raise exception using errcode = '55000', message = 'Phase 1B-2A integrated guard catalog state is partial or incompatible';
    end if;

    if actual_signature = expected_post_signature then
        return;
    end if;
    if actual_signature is distinct from expected_pre_signature then
        raise exception using errcode = '55000', message = 'Phase 1B-2A integrated guard requires the exact post-activation state', detail = format('expected signature %s, found %s', expected_pre_signature, actual_signature);
    end if;
    ...
    if (select count(*) from pg_trigger as trigger_value join pg_class as relation on relation.oid = trigger_value.tgrelid join pg_namespace as namespace on namespace.oid = relation.relnamespace where namespace.nspname='public' and relation.relname in ('security_events', 'business_audit_events') and trigger_value.tgname in ('trg_security_events_contract', 'trg_business_audit_events_contract') and not trigger_value.tgisinternal) <> 2
       ...
       or has_table_privilege('service_role', 'public.canonical_event_catalog_projection', 'select') then
        raise exception using errcode = '55000', message = 'Phase 1B-2A integrated guard prerequisites are incomplete or unsafe';
    end if;
...
$guard$;
    ...
    if actual_signature is distinct from expected_post_signature then
        raise exception using errcode = '55000', message = 'Phase 1B-2A integrated guard final semantic signature is incompatible', detail = format('expected signature %s, found %s', expected_post_signature, actual_signature);
    end if;
end
$migration$;
