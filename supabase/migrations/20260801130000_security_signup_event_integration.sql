-- Phase 1B-2B: activate only the three canonical runtime signup events.
-- Projection metadata only: no event backfill, business DML, guard, trigger, or writer changes.
...
declare
    ...
    selected_names constant text[] := array['security.signup.completed', 'security.signup.failed', 'security.signup.started'];
    expected_pre_signature constant text := '993b1de965a1791a2a84ccff5fcfbdf9';
    expected_post_signature constant text := '371c7010a0553c7953708dea164ed0bc';
    expected_pre_integrated constant text[] := array['security.login.failed', 'security.login.started', 'security.login.succeeded', 'security.logout.completed'];
    expected_post_integrated constant text[] := array['security.login.failed', 'security.login.started', 'security.login.succeeded', 'security.logout.completed', 'security.signup.completed', 'security.signup.failed', 'security.signup.started'];
...
begin
    ...
    select array_agg(event_name order by event_name) into integrated_names from public.canonical_event_catalog_projection where integrated;

    if actual_signature = expected_post_signature then
        if integrated_names is distinct from expected_post_integrated
           ...
           or (select count(*) from public.canonical_event_catalog_projection where integrated) <> 7 then
            raise exception using errcode = '55000', message = 'signup integration post-state is incompatible';
        end if;
        return;
    end if;

    if actual_signature is distinct from expected_pre_signature
       or integrated_names is distinct from expected_pre_integrated
       or (select count(*) from public.canonical_event_catalog_projection) <> 158
       or (select count(*) from public.canonical_event_catalog_projection where lifecycle_status = 'planned') <> 150
       or (select count(*) from public.canonical_event_catalog_projection where lifecycle_status = 'deferred') <> 8
       or (select count(*) from public.canonical_event_catalog_projection where writable) <> 144
       or (select count(*) from public.canonical_event_catalog_projection where integrated) <> 4
       or (select count(*) from public.canonical_event_catalog_projection where event_name = any(selected_names) and lifecycle_status = 'planned' and writable and not integrated) <> 3 then
        raise exception using errcode = '55000', message = 'signup integration pre-state is partial or incompatible', detail = format('expected signature %s, found %s', expected_pre_signature, actual_signature);
    end if;

    update public.canonical_event_catalog_projection set integrated = true
      where event_name = any(selected_names) and not integrated;
    get diagnostics changed_rows = row_count;
    if changed_rows <> 3 then
        raise exception using errcode = '55000', message = 'signup integration did not activate exactly three definitions';
    end if;

    select array_agg(event_name order by event_name) into integrated_names from public.canonical_event_catalog_projection where integrated;
    if integrated_names is distinct from expected_post_integrated
       or (select count(*) from public.canonical_event_catalog_projection where lifecycle_status = 'planned' and not integrated) <> 143
       or (select count(*) from public.canonical_event_catalog_projection where writable and not integrated) <> 137 then
        raise exception using errcode = '55000', message = 'signup integration final catalog counts are incompatible';
    end if;
    actual_signature := pg_temp.canonical_event_semantic_signature();
    if actual_signature is distinct from expected_post_signature then
        raise exception using errcode = '55000', message = 'signup integration final semantic signature is incompatible', detail = format('expected signature %s, found %s', expected_post_signature, actual_signature);
    end if;
end
$migration$;
