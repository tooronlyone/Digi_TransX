...
| Latest capability-status reconciliation | 2026-08-08; Phase 1A through Phase 1B-2C1 are implemented in main and authorized shared TEST. Phase 1B-2C2 is implemented on its feature branch only and has not been applied to shared TEST or production. Durable server-session runtime remains unwired and undeployed. |
...
| Phase | Phase 0 registry; Phase 1A through Phase 1B-2C1 implemented in main and authorized shared TEST; Phase 1B-2C2 implemented on feature branch only; durable-session runtime integration remains Planned |
...
The TEST database has 46 public tables and two public views. It contains the capabilities through the Phase 1B-1 canonical event ACL-hardening migration, assessed by objects and constraints rather than by a migration-history table:
...
- Phase 1B-2C2 on `feature/phase-1b2c2-trusted-device-hardening` stores only SHA-256 token digests, preserves stable device/user ownership, enforces absolute expiry and explicit revocation, and rotates credentials under a row lock. A trusted device remains recognition evidence only, never an authenticated session.
...
| --- | --- | --- | --- | --- |
| Signup | Phase 1B-2B implemented in main and shared TEST | [`signup()`](../backend/auth/routes.py) commits anonymous `security.signup.started` before external Auth creation. It then commits public user/profile, successful `login_activity`, trusted device, and `security.signup.completed` together; Flask session, CSRF, and device cookie follow only after commit. Failure records one anonymous coarse `security.signup.failed` code and uses bounded Auth compensation/reconciliation. | Main and shared TEST: `security_events`, `auth.users`, `users`, one role-profile table, `login_activity`, `trusted_devices`; shared TEST has seven integrated auth definitions and empty event tables after rolled-back verification | Exactly `security.signup.started`, `.failed`, and `.completed` are integrated. GPS and Email/SMS OTP remain Planned and unintegrated; provider messages, identity fields, device token, and form values are excluded from canonical evidence. |
| Password login | Phase 1B-2A bounded evidence implemented in main and shared TEST; broader auth remains unstandardized | [`login()`](../backend/auth/routes.py) durably emits server-owned `security.login.started` before provider verification, then keeps terminal `login_activity`, successful `last_login_at`, trusted-device persistence, and the terminal event in one separate caller transaction. The Flask session is created only after that transaction commits. | Main: `security_events`, `login_activity`, `users.last_login_at`, `trusted_devices`, Flask cookie; shared TEST has seven integrated auth definitions and the integrated-state guard | Blocked, unknown, and wrong-password attempts have one indistinguishable public 401 response while internal canonical result codes remain coarse and distinct. No GPS, mandatory Email OTP, risk engine, device/session limits, or high-risk restrictions. |
| Logout | Phase 1B-2A bounded evidence implemented in main and shared TEST | [`logout()`](../backend/auth/routes.py) validates authenticated CSRF, derives the actor before clearing, attempts `security.logout.completed`, and always clears the local session if audit persistence fails. | Main: `security_events` plus client cookie; shared TEST has seven integrated auth definitions and the integrated-state guard | No server session exists to revoke; current device token remains trusted. Anonymous/invalid-CSRF requests emit no logout event. |
...
| Trusted device / fast login | Phase 1B-2C2 implemented on feature branch only | [`trusted_device_service.py`](../backend/auth/trusted_device_service.py) is the single transaction-participating owner for generation, SHA-256 digestion, resolution, creation, rotation, and revocation. The HttpOnly SameSite=Lax cookie is bounded to at most 30 days. Legacy MPIN authentication routes fail closed and clear the device cookie. | `trusted_devices`, `users.mpin_hash`, `users.mpin_enabled` | Secure MPIN unlock/step-up remains Planned for 2C3; device UI remains Planned for 2C6. |
| Flask session | Verified existing but unstandardized; Phase 1B-2C1 foundation is implemented but unused | [`build_auth_success_response()`](../backend/auth/helpers.py#L357) sets `user_id`, CSRF token, and `last_active_at` in Flask's signed cookie. [`login_required()`](../backend/auth/helpers.py#L286) refreshes `last_active_at` on every authenticated request. | Signed cookie remains the runtime owner; main and shared TEST contain unused `user_sessions`, and main contains the unused [`auth/session_service.py`](../backend/auth/session_service.py) owner | No durable token issuance, replay detection, revocation enforcement, seven-day inactivity enforcement, or 30-day absolute-lifetime enforcement. Polling requests currently count as cookie activity. |
...
- Nineteen forward migrations exist under [`supabase/migrations`](../supabase/migrations) in main. `20260801150000_durable_server_session_foundation.sql` is merged through commit `682b2dcf53ef10b3f56b6ed6b2395955442a0904` and applied to authorized shared TEST with Git-blob SHA-256 `11b32c6d7cbd70dd16fb3f2854d3eb615d922fcd58049c0462917f635e9ada20`.
...
Shared TEST contains empty `security_events` and `business_audit_events` tables plus the 170-row `canonical_event_catalog_projection`; 162 definitions are Planned, 8 Deferred QR, 156 writable, and exactly seven auth definitions are `integrated=true`. Applying `20260801140000_device_session_mpin_event_contracts.sql` (Git-blob SHA-256 `653a5859cefea10705b8b547519f73892059294d11e9da5e773f1e278d777bba`) changed the semantic signature from `371c7010a0553c7953708dea164ed0bc` to `3d9b730408336c82629c25342ddc7ea2`. `event_outbox` remains absent. `service_role` has exactly `DELETE`, `INSERT`, and `SELECT` on each event table and zero projection access; `anon`, `authenticated`, and `PUBLIC` have zero event or projection access. Only integrated+writable definitions are insertable; 149 writable-but-unintegrated Planned definitions, all six Operations definitions, and all eight Deferred QR definitions are rejected. Rolled-back verification left both event tables empty and created no TEST Auth account or business row.

The Phase 1B-2C0 current-final projection is implemented in main and authorized shared TEST with 170 rows: 162 Planned, 8 Deferred QR, 156 writable, seven integrated auth definitions, 155 Planned-unintegrated definitions, 149 writable-unintegrated definitions, and six Operations definitions. Its semantic signature is `3d9b730408336c82629c25342ddc7ea2`. The added/formalized device, session, trusted-device, and MPIN definitions remain unintegrated, and the migration created no runtime behavior.
...
- [`backend/events/catalog.py`](../backend/events/catalog.py) remains the only machine-readable catalog owner. On the Phase 1B-2C2 feature branch, 10 Planned security definitions are integrated: the prior seven auth definitions plus `security.trusted_device.added`, `.removed`, and `.rotated`. The other 152 Planned definitions remain unintegrated. Shared TEST remains at the prior seven-event state because this migration was not applied there.
...
- Migration/fresh schema produces zero event rows. On this feature branch, the integrated guard admits the 10 integrated definitions and rejects the remaining 146 writable-but-unintegrated Planned definitions; shared TEST remains unchanged at seven integrated definitions.
...
- Phase 1B-2C2 feature verification passed 699 PostgreSQL tests with zero failures/skips; explicit no-DB mode passed 308 with 391 expected PostgreSQL skips. Migration `20260801160000_trusted_device_hardening.sql` has SHA-256 `016c99fbf924e6eb578f563f6611a757fe1228293e8bc31f40e1282d63753609`; fresh/sequential/reapplication/corrupt-state, ACL, auth rollback, MPIN fail-closed, and prior-domain suites passed without applying to shared TEST.
...
| Canonical event foundation | `backend/events` catalog/contract/writers | Shared TEST `security_events`, `business_audit_events`, and `canonical_event_catalog_projection` | Shared TEST is at the applied empty-table, 170-definition, seven-integration state with narrow ACL | Foundation plus seven integrated auth definitions and seventeen unintegrated Phase 1B-2C0 contracts in main and shared TEST | Continue only through separately reviewed domain-owner changes using the existing writer/caller transaction | Any direct route table write, client identity/time, second catalog, partial activation, or guard regression |
...
| MPIN/password/reset/device | Auth/profile routes plus canonical trusted-device service | Supabase Auth plus OTP/reset and hardened `trusted_devices` | Raw-token MPIN authentication bypass is disabled fail-closed; secure MPIN data is retained | Trusted-device added/removed/rotated integrated on feature branch | MPIN unlock/step-up Planned for 2C3; revocation triggers Planned for 2C4 |
...
7. **Phase 1B-2C1 — durable server-session foundation (Implemented in main and authorized shared TEST; not deployed)** — one zero-row `user_sessions` table and one unused session service own the digest-only token model, genuine activity, lock, expiry, rotation, and revocation state. The rollout added one table, four indexes, nine table constraints, and one service-role policy; RLS is enabled, `PUBLIC`, `anon`, and `authenticated` have zero privileges, and `service_role` has exactly `INSERT`, `SELECT`, and `UPDATE` with no `DELETE`, `TRUNCATE`, `REFERENCES`, `TRIGGER`, or `MAINTAIN`. Runtime issuance/validation/refresh and cookie rotation, trusted-device runtime hardening/rotation, MPIN enrollment/unlock/lockout/reset/step-up, logout-all/remote and password/account-block revocation, seven-day genuine-inactivity enforcement, automatic locking, management APIs/dashboard, GPS, and Email/SMS OTP remain Planned. All seventeen Phase 1B-2C0 definitions remain unintegrated.
8. **Phase 1B-2C2 — trusted-device hardening (Implemented on feature branch only; not applied or deployed)** — migrates eligible 256-bit legacy credentials in place to SHA-256 digests, adds 30-day maximum absolute expiry plus revocation/rotation state, preserves stable IDs and same-user session binding, centralizes runtime ownership, integrates exactly the three existing trusted-device events, and disables legacy MPIN authentication bypass. Durable-session runtime remains Planned; secure MPIN unlock/step-up is Planned for 2C3; logout-all and password/reset/block revocation for 2C4; seven-day genuine inactivity for 2C5; management UI for 2C6; GPS and OTP remain Planned.
...
The catalog contains 162 Planned-lifecycle events plus 8 Deferred QR-payment events (170 combined), of which 156 are writable. On the Phase 1B-2C2 feature branch exactly 10 Planned security definitions are integrated: the prior seven auth names and the three trusted-device names. The other 152 Planned definitions remain unintegrated; 146 of those are writable. Shared TEST remains at the prior seven-event projection. The six Operations definitions remain non-writable and unintegrated.
