...
| Latest capability-status reconciliation | 2026-08-12; Phase 1A through Phase 1B-2C3B1, its bounded security corrections, and genuine-session-activity runtime are implemented in main and authorized shared TEST. Phase 1B-2C3B2 MPIN/access-lock frontend UX is implemented only on its feature branch pending review and merge. No production rollout or deployment occurred. |
...
| Phase | Phase 0 registry; Phase 1A through Phase 1B-2C3B1 implemented in main/shared TEST; Phase 1B-2C3B2 frontend implemented on its feature branch only; production remains undeployed |
...
| --- | --- | --- | --- | --- |
| Signup | Phase 1B-2C3B1 durable issuance implemented in main and shared TEST | [`signup()`](../backend/auth/routes.py) commits anonymous `security.signup.started` before external Auth creation. Its terminal caller-owned transaction atomically persists the public user/profile, successful `login_activity`, trusted-device mutation and event, durable session, `security.signup.completed`, and `security.session.issued`; session/device/access-proof cookies and CSRF state issue only after commit. Mandatory failure rolls back terminal success evidence and uses bounded Auth compensation/reconciliation. | Main and shared TEST: `security_events`, `auth.users`, `users`, one role-profile table, `login_activity`, hardened `trusted_devices`, `user_sessions`; shared TEST has 21 integrated definitions and empty event tables after rolled-back verification | GPS and Email/SMS OTP security flows remain Planned and unintegrated; provider messages, identity fields, credentials, and form values are excluded from canonical evidence. |
...
| Secure MPIN / software access lock | Phase 1B-2C3B1 backend implemented in main and authorized shared TEST; Phase 1B-2C3B2 frontend implemented on its feature branch only; not deployed | [`mpin_service.py`](../backend/auth/mpin_service.py) owns exact four-ASCII-digit validation, unique 32-byte salt, mandatory decoded 32-byte environment pepper with no default, versioned `hashlib.scrypt` verification, atomic attempt state, and permanent fifth-failure lockout. [`login_required()`](../backend/auth/helpers.py) centrally distinguishes invalid authentication from a valid locked session and permits only status, unlock, password unlock, and logout while locked. The feature-branch React owner recognizes only structured `423 access_locked`, blocks protected content, offers MPIN/password unlock and logout, and exposes one canonical management component to the three eligible role surfaces. Automatic replay fails closed except for at most one exact, bodyless same-origin `GET /api/platform/terms/current`, whose local handler is SELECT-only and provider-free; every other route remains component-owned. | One user-owned `mpin_credentials` row; digest-only session-bound `dtx_access_proof` state in `user_sessions`; raw proof only in memory and an HttpOnly non-persistent cookie; shared TEST has zero credentials/sessions | Eligible roles are exactly `logistics_provider`, `service_seeker`, and `everyday_user`. The proof has an eight-hour maximum server lifetime. Feature-branch status is not a main/shared-TEST rollout claim. High-risk step-up, logout-all/device dashboard, expiry-event processing, GPS, and OTP remain Planned. |
...
- The legacy MPIN authentication bypass remains disabled. Secure enrollment/unlock/password-unlock/change/disable/reset, the separate access proof, `security.session.access_locked`, permanent fifth-failure MPIN lockout, and trusted visible-interaction `.refreshed` activity are implemented in main and authorized shared TEST. The bounded MPIN/access-lock frontend UX is implemented only on the Phase 1B-2C3B2 feature branch. Action-bound MPIN step-up, `.expired_inactivity`, logout-all, password/reset/block mass revocation, the device/session dashboard, GPS, and Email/SMS OTP security flows remain Planned.
...
11. **Phase 1B-2C3B2 — secure MPIN frontend and software-lock UX (Implemented on feature branch only; not merged or deployed)** — one centralized React access state owner derives seven states from `/auth/me` and canonical MPIN status, recognizes only `423` with `code=access_locked`, blocks and inerts protected content, keeps CSRF logout and password recovery available, and replays at most one query-free same-origin safe GET after authoritative unlock. One accessible masked four-digit control and one canonical management component serve transporter, Business/service-seeker, and Everyday surfaces; other roles receive no management route. MPIN/password values remain component/process memory only and never enter URLs, browser persistence, analytics, or logs. Legacy fast-login callers are removed. High-risk step-up, logout-all/device dashboard, expiry-event processing, GPS, and OTP remain Planned. Catalog totals and signature are unchanged.
