...
| Latest capability-status reconciliation | 2026-08-09; Phase 1A through Phase 1B-2C2 are implemented in main and authorized shared TEST. Phase 1B-2C3A durable server-session runtime is implemented only on `feature/phase-1b2c3a-durable-session-runtime`; it has not been merged, applied to shared TEST, or deployed. |
...
| Phase | Phase 0 registry; Phase 1A through Phase 1B-2C2 implemented in main and authorized shared TEST; Phase 1B-2C3A implemented on its feature branch only |
...
The TEST database has 46 public tables and two public views. It contains the capabilities through the Phase 1B-2C2 trusted-device-hardening migration, assessed by objects and constraints rather than by an application-owned migration-history table:
...
| --- | --- | --- | --- | --- |
| Signup | Phase 1B-2B implemented in main and shared TEST | [`signup()`](../backend/auth/routes.py) commits anonymous `security.signup.started` before external Auth creation. It then commits public user/profile, successful `login_activity`, trusted device, and `security.signup.completed` together; Flask session, CSRF, and device cookie follow only after commit. Failure records one anonymous coarse `security.signup.failed` code and uses bounded Auth compensation/reconciliation. | Main and shared TEST: `security_events`, `auth.users`, `users`, one role-profile table, `login_activity`, hardened `trusted_devices`; shared TEST has ten integrated definitions and empty event tables after rolled-back verification | Exactly `security.signup.started`, `.failed`, and `.completed` are signup-integrated. GPS and Email/SMS OTP remain Planned and unintegrated; provider messages, identity fields, device credentials, and form values are excluded from canonical evidence. |
| Password login | Phase 1B-2A evidence in main/shared TEST; Phase 1B-2C3A runtime on feature branch only | [`login()`](../backend/auth/routes.py) commits `security.login.started` before provider verification, then atomically persists successful activity, `last_login_at`, trusted-device mutation, one durable session, and terminal/session events. Cookies issue only after commit. | Feature branch: `security_events`, `login_activity`, `users`, hardened `trusted_devices`, `user_sessions`; shared TEST remains at ten integrations | Public failure normalization is unchanged. GPS, mandatory Email OTP, risk engine, limits, and high-risk restrictions remain Planned. |
| Logout | Phase 1B-2A evidence in main/shared TEST; Phase 1B-2C3A runtime on feature branch only | [`logout()`](../backend/auth/routes.py) validates durable authentication and CSRF, locks and revokes exactly the current session, writes `security.session.revoked` with `security.logout.completed`, then always clears local auth state. | Feature branch: `user_sessions`, `security_events`, durable/device/Flask cookies; shared TEST remains at ten integrations | Other sessions are unaffected; logout-all remains Planned for 2C4. Anonymous/invalid-CSRF requests emit no logout event. |
...
| Flask session | Phase 1B-2C3A feature branch: non-authoritative compatibility state only | [`build_auth_success_response()`](../backend/auth/helpers.py) retains only CSRF and presentation activity state. [`login_required()`](../backend/auth/helpers.py) requires the durable-session and exact trusted-device cookies, then derives identity/role from PostgreSQL. | Feature branch: `user_sessions` is authoritative; Flask cookie contains no user ID, role, profile ID, email, or CNIC | Passive `/auth/me` does not update durable genuine activity. Inactivity processing remains Planned for 2C5. |
...
Shared TEST contains empty `security_events` and `business_audit_events` tables plus the 170-row `canonical_event_catalog_projection`; 162 definitions are Planned, 8 Deferred QR, 156 writable, and exactly ten definitions are `integrated=true`. Phase 1B-2C0 changed the semantic signature from `371c7010a0553c7953708dea164ed0bc` to `3d9b730408336c82629c25342ddc7ea2`; Phase 1B-2C2 then changed it to `11043982605bef207d3b9a5626bd86d8` by integrating only `security.trusted_device.added`, `.rotated`, and `.removed`. `event_outbox` remains absent. `service_role` has exactly `DELETE`, `INSERT`, and `SELECT` on each event table and zero projection access; `anon`, `authenticated`, and `PUBLIC` have zero event or projection access. Only integrated+writable definitions are insertable; 146 writable-but-unintegrated Planned definitions, all six Operations definitions, and all eight Deferred QR definitions are rejected. Rolled-back verification left both event tables empty and created no TEST Auth account or business row.

The Phase 1B-2C2 current-final projection is implemented in main and authorized shared TEST with 170 rows: 162 Planned, 8 Deferred QR, 156 writable, ten integrated definitions, 152 Planned-unintegrated definitions, 146 writable-unintegrated definitions, and six Operations definitions. Its semantic signature is `11043982605bef207d3b9a5626bd86d8`. The three trusted-device definitions are integrated; session and MPIN definitions remain unintegrated.

Phase 1B-2C0 added `security.session.issued`, `security.session.access_locked`, `security.trusted_device.rotated`, `security.mpin.enrolled`, `security.mpin.changed`, `security.mpin.disabled`, `security.mpin.unlock_succeeded`, `security.mpin.unlock_failed`, `security.mpin.locked`, `security.mpin.reset_completed`, `security.mpin.step_up_succeeded`, and `security.mpin.step_up_failed`; it formalized contracts for the existing `security.session.refreshed`, `security.session.expired_inactivity`, `security.session.revoked`, `security.trusted_device.added`, and `security.trusted_device.removed`. Phase 1B-2C2 integrated exactly the three trusted-device names; the fourteen session and MPIN definitions remain Planned, writable, and unintegrated.
...
Phase 1B-2C3A is implemented on `feature/phase-1b2c3a-durable-session-runtime` only. `user_sessions` is the sole runtime authentication authority: full password signup/login atomically creates one digest-only session bound to the exact active same-user trusted device, then returns the raw opaque credential only in the separate HttpOnly `dtx_session_token` cookie (SameSite=Lax, path `/`, Secure outside explicit local development, 30-day bound). Flask's signed session retains only non-authoritative CSRF and presentation activity state; browser-supplied user ID, role, and profile identity are ignored. `/auth/me` validates without refreshing `last_genuine_activity_at`. Current-session logout locks and revokes exactly that session and writes `security.session.revoked` with `security.logout.completed` in the same transaction, while local cookies are cleared even if persistence fails.

The feature-only forward migration `20260801170000_durable_session_runtime_events.sql` (working-tree SHA-256 `fe4d8440ad683396b31da7ec8da0e335ceaa8936c1fb1c4d5f51a24058f57aeb`) integrates exactly `security.session.issued` and `security.session.revoked`. The feature projection remains 170 definitions and 156 writable definitions, with 12 integrated, 150 Planned-unintegrated, 144 writable-unintegrated, and 8 Deferred QR definitions; its semantic signature is `87c1377e1404933c69b1a90ac9962937`. Shared TEST intentionally remains at the Phase 1B-2C2 10-integrated signature above. `security.session.refreshed`, `.expired_inactivity`, and `.access_locked` remain Planned and rejected, as do all MPIN events. MPIN runtime remains Planned for 2C3B; logout-all and security/password/reset/account-block revocation for 2C4; seven-day genuine-inactivity processing for 2C5; the session/device dashboard for 2C6; GPS and mandatory Email/SMS OTP remain Planned.
...
| `20260801170000_durable_session_runtime_events.sql` | Activates only durable-session issuance and current-session revocation event contracts; no table change or fixtures | Implemented on the Phase 1B-2C3A feature branch only; not applied to shared TEST; working-tree SHA-256 `fe4d8440ad683396b31da7ec8da0e335ceaa8936c1fb1c4d5f51a24058f57aeb`; signature `11043982605bef207d3b9a5626bd86d8` to `87c1377e1404933c69b1a90ac9962937` |
...
- [`backend/events/catalog.py`](../backend/events/catalog.py) remains the only machine-readable catalog owner. Main and authorized shared TEST have 10 integrated Planned security definitions: the prior seven auth definitions plus `security.trusted_device.added`, `.removed`, and `.rotated`. The other 152 Planned definitions remain unintegrated.
...
- Migration/fresh schema produces zero event rows. In main and authorized shared TEST, the integrated guard admits the 10 integrated definitions and rejects the remaining 146 writable-but-unintegrated Planned definitions.
...
- Signup runtime integration is implemented in main and shared TEST: start commits before external provider creation; public user, correct role profile, success activity, trusted-device mutation, and completed evidence share the terminal public-database transaction; session, CSRF, and device cookie issue only after commit. Failure uses one anonymous `security.signup.failed` event with an approved coarse result code. Cleanup deletes only an Auth identity provably created by that request; ambiguous or failed cleanup records `reconciliation_required`. Canonical metadata never includes password, token, OTP, CNIC, email, provider error text, or another sensitive value. MPIN, password/reset, OTP, GPS, risk, session lifecycle, account lock/unlock, business, analytics, and Operations integrations remain Planned.
...
| --- | --- | --- | --- | --- | --- |
| Password login/logout | Auth route; no event aliases | Supabase Auth verification plus `users.last_login_at`, `login_activity`, `security_events`, `trusted_devices`, and session response | Login-start commits before provider verification. Terminal activity/event, genuine-success timestamp, and trusted-device mutation use one separate caller-owned DB transaction; session follows its commit. Logout audit owns one DB transaction and session clear is fail-safe. | Exactly four Phase 1B-2A `security.login.*`/`security.logout.completed` events | **Main-integrated and TEST-guarded**; no cross-system atomicity is claimed for Supabase Auth verification |
| Signup | Auth route in main | Supabase Auth plus `users`, one role profile, `login_activity`, `trusted_devices`, and `security_events` | Start commits before external Auth. Public user/profile, activity, trusted device, and completed event share one public transaction; session follows commit. Proven Auth compensation is bounded to the request-owned identity. | `security.signup.started`, `.failed`, `.completed` | **Main-integrated and TEST-guarded** |
...
Phase 1B-1 does not change a domain owner. Phase 1B-2A attaches the existing cursor-level security writer to four password-login/logout outcomes. Phase 1B-2B attaches three signup outcomes in main and shared TEST. Phase 1B-2C2 attaches the three trusted-device outcomes in main and authorized shared TEST. Phase 1B-2C3A attaches issued/revoked session outcomes only on its feature branch; GPS, OTP, and MPIN runtime remain future bounded changes.
...
6. **Phase 1B-2C0 — device/session/MPIN event contracts (Implemented in main and authorized shared TEST; not deployed)** — migration `20260801140000_device_session_mpin_event_contracts.sql` added twelve contracts and formalized five existing contracts. At Phase 1B-2C0 completion all seventeen were unintegrated; Phase 1B-2C2 later integrated only the three trusted-device definitions. Durable-session runtime, software MPIN unlock, high-risk payment/action MPIN step-up, seven-day genuine-inactivity enforcement, GPS, Email OTP, and SMS OTP remain Planned, and no payment or wallet behavior changed.
7. **Phase 1B-2C1 — durable server-session foundation (Implemented in main and authorized shared TEST; not deployed)** — one zero-row `user_sessions` table and one unused session service own the digest-only token model, genuine activity, lock, expiry, rotation, and revocation state. The rollout added one table, four indexes, nine table constraints, and one service-role policy; RLS is enabled, `PUBLIC`, `anon`, and `authenticated` have zero privileges, and `service_role` has exactly `INSERT`, `SELECT`, and `UPDATE` with no `DELETE`, `TRUNCATE`, `REFERENCES`, `TRIGGER`, or `MAINTAIN`. Runtime issuance/validation/refresh, replacement of Flask signed-cookie authentication, MPIN enrollment/unlock/lockout/reset/step-up, logout-all/remote and password/account-block revocation, seven-day genuine-inactivity enforcement, automatic locking, management APIs/dashboard, GPS, and Email/SMS OTP remain Planned. Fourteen Phase 1B-2C0 session/MPIN definitions remain unintegrated after Phase 1B-2C2 integrated only the three trusted-device definitions.
8. **Phase 1B-2C2 — trusted-device hardening (Implemented in main and authorized shared TEST; not deployed)** — integration commit `eb10d8eff93499a67e424bea5a60fd0245d46c4f` and migration `20260801160000_trusted_device_hardening.sql` (SHA-256 `016c99fbf924e6eb578f563f6611a757fe1228293e8bc31f40e1282d63753609`) migrated 31 eligible rows in place to exact 32-byte SHA-256 digests while preserving stable IDs, user ownership, and same-user session binding. It removed raw-token storage, added 30-day maximum absolute expiry plus revocation/locked-rotation state, centralized runtime ownership, integrated exactly the three trusted-device events, and disabled the legacy MPIN authentication bypass while retaining MPIN data. Durable-session issuance/enforcement and replacement of Flask signed-cookie authentication remain Planned; secure MPIN enrollment/unlock/attempt limits/lockout/reset/step-up and action/resource-bound grants are Planned for 2C3; logout-all/remote management and password/reset/block revocation for 2C4; seven-day genuine inactivity for 2C5; the security dashboard for 2C6; GPS and Email/SMS OTP remain Planned.
9. **Phase 1B-2C3A — durable server-session runtime authority (Implemented on feature branch only; not merged, not applied to shared TEST, not deployed)** — password signup/login issue one digest-only session bound to the exact active trusted device in the terminal caller-owned transaction; all protected routes resolve current identity and role from that durable row and current user record. Flask signed state is CSRF/presentation-only. Current logout revokes exactly its locked session. Exactly `security.session.issued` and `.revoked` advance the feature projection to 12 integrations and signature `87c1377e1404933c69b1a90ac9962937`. MPIN 2C3B, logout-all/revocation triggers 2C4, inactivity processing 2C5, dashboard 2C6, GPS, and OTP remain Planned.
...
9. **Security** — observation first; risk shadow mode and alerts/cases; later Email OTP after provider/domain readiness. Durable-session runtime, secure MPIN runtime, revocation integrations, and device/session management remain later dedicated security workstreams.
...
The catalog contains 162 Planned-lifecycle events plus 8 Deferred QR-payment events (170 combined), of which 156 are writable. Main and authorized shared TEST have exactly 10 integrated Planned security definitions: the prior seven auth names and the three trusted-device names. The other 152 Planned definitions remain unintegrated; 146 of those are writable. The six Operations definitions remain non-writable and unintegrated. The final semantic signature is `11043982605bef207d3b9a5626bd86d8`, advanced from `3d9b730408336c82629c25342ddc7ea2` only by the three trusted-device integrations.
