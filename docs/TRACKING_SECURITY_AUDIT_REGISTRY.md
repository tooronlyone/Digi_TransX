...
| Latest capability-status reconciliation | 2026-08-03; Phase 1B-2A activation and integrated-state guard are applied to authorized shared TEST and merged/pushed to main; four auth events are integrated and guarded |
...
| Phase | Phase 0 registry; Phase 1A containment implemented; schema-trigger/RLS baseline, Phase 1B-1 foundation/ACL, and Phase 1B-2A login/logout activation/guard implemented in main and authorized shared TEST; every other runtime integration remains Planned |
...
- Fifteen forward migrations exist under [`supabase/migrations`](../supabase/migrations). The Phase 1B-1 foundation/ACL, Phase 1B-2A activation, and forward integrated-guard correction are applied to shared TEST and merged to main.
...
- On this correction branch, [`backend/events/catalog.py`](../backend/events/catalog.py) remains the only machine-readable catalog owner: 150 Planned-lifecycle names plus 8 Deferred QR names. The new general terminal signup-failure definition remains distinct from signup GPS and Email-OTP failure definitions. It is Planned, writable, and `integrated=False`; its only permitted metadata is one coarse `result_code` from `validation_failed`, `account_conflict`, `provider_unavailable`, `persistence_failed`, or `reconciliation_required`. Phase 1B-2A still marks exactly four auth definitions `integrated=True`; the other 146 Planned definitions and all 8 Deferred definitions remain unintegrated. Deferred QR and the six Operations names remain non-writable.
...
- Migration/fresh schema produced zero event rows. On this correction branch, the integrated guard continues to admit only the four integrated definitions and rejects all 140 writable-but-unintegrated Planned definitions, including the new signup failure definition, plus Operations and Deferred QR definitions; it preserves existing envelope, actor, idempotency, append-only, and ACL guarantees.
...
Phase 1B-2A is implemented in main and authorized shared TEST. Exactly `security.login.started`, `security.login.failed`, `security.login.succeeded`, and `security.logout.completed` are integrated and insertable. This branch-only catalog correction adds one unintegrated general signup-failure definition; 146 other Planned-lifecycle definitions and all 8 Deferred QR definitions remain unintegrated.
...
| Canonical event foundation | `backend/events` catalog/contract/writers | Shared TEST `security_events`, `business_audit_events`, and `canonical_event_catalog_projection` | Shared TEST remains at the applied empty-table, 157-definition state with narrow ACL; this correction branch adds a 158th unintegrated definition only | Foundation plus exactly four integrated auth definitions | Continue only through separately reviewed domain-owner changes using the existing writer/caller transaction | Any direct route table write, client identity/time, second catalog, partial activation, or guard regression |
...
On this correction branch, the canonical catalog contains 150 Planned-lifecycle events plus 8 Deferred QR-payment events (158 combined). Deferred events are reported separately and excluded from the Planned total. Exactly four Planned-lifecycle security definitions are integrated by Phase 1B-2A; the other 146 Planned definitions, including the new signup-failure definition, and all 8 Deferred definitions remain unintegrated. This catalog correction is branch-only and is not applied to shared TEST.
