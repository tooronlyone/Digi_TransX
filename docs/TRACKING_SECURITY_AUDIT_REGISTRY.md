...
| Secure MPIN / software access lock | Phase 1B-2C3B1 backend implemented in main and authorized shared TEST; Phase 1B-2C3B2 frontend implemented in repository main; not deployed | [`mpin_service.py`](../backend/auth/mpin_service.py) owns exact four-ASCII-digit validation, unique 32-byte salt, mandatory decoded 32-byte environment pepper with no default, versioned `hashlib.scrypt` verification, atomic attempt state, and permanent fifth-failure lockout. [`login_required()`](../backend/auth/helpers.py) centrally distinguishes invalid authentication from a valid locked session and permits only status, unlock, password unlock, and logout while locked. The canonical seven-state React owner recognizes only structured `423` with `code=access_locked`, requires full login for ordinary `401`, blocks and inerts protected content, offers MPIN/password unlock and canonical logout, and exposes one shared management component to the three eligible role surfaces. Automatic replay is generation-bound, one-shot, path-only, and fail-closed: its exact allowlist contains only bodyless same-origin `GET /api/platform/terms/current`, whose local handler is SELECT-only and provider-free; every other route remains component-owned. | One user-owned `mpin_credentials` row; digest-only session-bound `dtx_access_proof` state in `user_sessions`; raw proof only in memory and an HttpOnly non-persistent cookie; shared TEST has zero credentials/sessions. MPIN/password frontend values remain non-persistent process state. | Eligible roles are exactly `logistics_provider`, `service_seeker`, and `everyday_user`. The proof has an eight-hour maximum server lifetime. Hassan's real-Chrome responsive and physical-keyboard verification explicitly passed. High-risk step-up, logout-all/device dashboard, expiry-event processing, GPS, and OTP remain Planned. No production deployment occurred. |
...
- The legacy MPIN authentication bypass remains disabled. Secure enrollment/unlock/password-unlock/change/disable/reset, the separate access proof, `security.session.access_locked`, permanent fifth-failure MPIN lockout, and trusted visible-interaction `.refreshed` activity are implemented in main and authorized shared TEST. The bounded Phase 1B-2C3B2 MPIN/access-lock frontend UX and responsive/navigation corrections are implemented in repository main without any shared-TEST/database change. Action-bound MPIN step-up, `.expired_inactivity`, logout-all, password/reset/block mass revocation, the device/session dashboard, GPS, and Email/SMS OTP security flows remain Planned.
...
Before the Phase 1B-2C3B1 rollout, shared TEST had 12 integrations and semantic signature `87c1377e1404933c69b1a90ac9962937`. Main and shared TEST now have empty `security_events` and `business_audit_events`, 170 catalog rows, 162 Planned, 8 Deferred QR, 156 writable, 21 integrated, 141 Planned-unintegrated, 135 writable-unintegrated, six Operations, and signature `b57a59369062e678a7b269cd61d4e01e`. The rollout added the 2C3B1 access-lock/seven non-step-up MPIN integrations and exactly `security.session.refreshed`; `event_outbox` remains absent, and ACL/append-only guard behavior is unchanged.
