...
| Latest capability-status reconciliation | 2026-08-03; Phase 1B-2A activation exists in local source and shared TEST, but origin/main is unadvanced and rollout is blocked by integrated-state guard hardening on `fix/phase-1b2a-integrated-event-guard` |
...
| Phase | Phase 0 registry; Phase 1A containment implemented; schema-trigger/RLS baseline and Phase 1B-1 foundation/ACL implemented in repository and shared TEST; Phase 1B-2A login/logout activation exists in local source and shared TEST, but rollout is blocked pending the correction-branch integrated-state guard; every other runtime integration remains Planned |
...
| Password login | Phase 1B-2A bounded evidence implemented in local source; broader auth remains unstandardized | [`login()`](../backend/auth/routes.py) durably emits server-owned `security.login.started` before provider verification, then keeps terminal `login_activity`, successful `last_login_at`, trusted-device persistence, and the terminal event in one separate caller transaction. The Flask session is created only after that transaction commits. | Local source: `security_events`, `login_activity`, `users.last_login_at`, `trusted_devices`, Flask cookie; shared TEST activation has four integrated rows but awaits guard correction | Blocked, unknown, and wrong-password attempts have one indistinguishable public 401 response while internal canonical result codes remain coarse and distinct. No GPS, mandatory Email OTP, risk engine, device/session limits, or high-risk restrictions. |
| Logout | Phase 1B-2A bounded evidence implemented in local source | [`logout()`](../backend/auth/routes.py) validates authenticated CSRF, derives the actor before clearing, attempts `security.logout.completed`, and always clears the local session if audit persistence fails. | Local source: `security_events` plus client cookie; shared TEST activation has four integrated rows but awaits guard correction | No server session exists to revoke; current device token remains trusted. Anonymous/invalid-CSRF requests emit no logout event. |
...
- Phase 1B-1 envelope schemas, canonical catalog/writers, explicit environment separation, RLS, idempotency, contract tests, and bounded `service_role` ACL hardening are **Implemented in the repository and shared TEST**. Phase 1B-2A activates exactly four password-login/logout definitions in local source and shared TEST; the correction branch hardens the guard before origin/main may advance. The other 145 Planned-lifecycle definitions and all 8 Deferred definitions remain unintegrated.
...
- Fifteen forward migrations exist under [`supabase/migrations`](../supabase/migrations). The Phase 1B-1 foundation/ACL and Phase 1B-2A activation are applied to shared TEST; the forward integrated-guard correction exists only on `fix/phase-1b2a-integrated-event-guard` and is not applied there.
...
Shared TEST contains empty `security_events` and `business_audit_events` tables plus the 157-row `canonical_event_catalog_projection`; exactly four login/logout definitions are `integrated=true`, with 149 Planned and 8 Deferred definitions and exactly 143 catalog-writable definitions. `event_outbox` remains absent. After the applied ACL correction, `service_role` has exactly `DELETE`, `INSERT`, and `SELECT` on each event table and zero projection access; `anon`, `authenticated`, and `PUBLIC` have zero event or projection access. The shared-TEST activation signature is `f5168975e0605fe0f7b84c1276a0082a`; a rolled-back direct-SQL probe showed its current guard still accepts writable-but-unintegrated names, so the forward correction is not applied and no shared-TEST event row was created.
...
- [`backend/events/catalog.py`](../backend/events/catalog.py) is the only machine-readable catalog owner: 149 Planned-lifecycle names plus 8 Deferred QR names. Phase 1B-2A marks exactly four auth definitions `integrated=True`; the correction branch requires both `writable=True` and `integrated=True` before runtime persistence. The other 145 Planned definitions and all 8 Deferred definitions remain unintegrated. Deferred QR and the six Operations names remain non-writable.
...
- `canonical_event_catalog_projection` is migration-owned enforcement metadata generated from the Python catalog, not a third event store or a second semantic owner. Its 157 rows project the exact name/version/category/domain/retention/lifecycle/writable/integrated tuple; deterministic bidirectional tests reject missing, extra, duplicate, or drifted definitions. Local source and shared TEST have exactly four integrated auth rows; the forward guard correction remains correction-branch only.
...
- Migration/fresh schema produced zero event rows. The correction branch changes the write contract from all 143 catalog-writable definitions to only the four integrated definitions; its direct-SQL matrix rejects the 139 writable-but-unintegrated Planned definitions, Operations, and Deferred QR events while preserving existing envelope, actor, idempotency, append-only, and ACL guarantees.
...
### 6.3 Phase 1B-2A bounded auth integration

Phase 1B-2A activation exists in local source and shared TEST, but origin/main is not advanced and rollout is blocked by integrated-state guard hardening on `fix/phase-1b2a-integrated-event-guard`. Exactly `security.login.started`, `security.login.failed`, `security.login.succeeded`, and `security.logout.completed` are integrated; 145 other Planned-lifecycle definitions remain unintegrated and all 8 Deferred QR definitions remain unintegrated.
...
- Signup, MPIN, trusted-device, password/reset, OTP, GPS, risk, session lifecycle, account lock/unlock, business, analytics, and Operations integrations remain Planned. Existing transitional `login_activity`, MPIN, trusted-device, CSRF, role, and password decisions are not replaced.

## 7. Security and login locked contract

Everything in this section is **Planned** unless explicitly cross-referenced as current.

Phase 1B-2A is that explicit cross-reference for `security.login.started`, `security.login.failed`, `security.login.succeeded`, and `security.logout.completed`: their lifecycle classification remains Planned in the locked 149 count, while their repository `integrated` flag is true. All other names below remain unintegrated.
...
| --- | --- | --- | --- | --- | --- | --- |
| Auth event evidence | Password-login/logout routes plus transitional helper | Shared TEST: `login_activity` 44 rows and empty `security_events`; repository activation unapplied | Phase 1B-2A integrates exactly four canonical events in repository while retaining existing activity evidence | Short committed login-start transaction, followed by one terminal transaction owning activity, success timestamp, trusted device where applicable, and terminal event; canonical payload excludes legacy identifier/IP/device-token fields | Any additional auth event, shared-TEST write, raw failure/identity field, duplicate start, or split terminal commit |
...
| Canonical event foundation | `backend/events` catalog/contract/writers | Shared TEST `security_events`, `business_audit_events`, and `canonical_event_catalog_projection` | Phase 1B-1 applied with empty tables, 157 definitions and narrow ACL; Phase 1B-2A activation remains unapplied | Foundation plus exactly four repository-integrated auth definitions | Continue only through separately reviewed domain-owner changes using the existing writer/caller transaction | Any direct route table write, client identity/time, second catalog, partial activation, or shared-TEST claim before application |
...
| --- | --- | --- | --- | --- | --- |
| Password login/logout | Auth route; no event aliases | Supabase Auth verification plus `users.last_login_at`, `login_activity`, `security_events`, `trusted_devices`, and session response | Login-start commits before provider verification. Terminal activity/event, genuine-success timestamp, and trusted-device mutation use one separate caller-owned DB transaction; session follows its commit. Logout audit owns one DB transaction and session clear is fail-safe. | Exactly four Phase 1B-2A `security.login.*`/`security.logout.completed` events | **Repository-integrated, TEST-unapplied**; no cross-system atomicity is claimed for Supabase Auth verification |
| Signup/MPIN/password/reset/device | Existing auth/profile routes and helpers | Supabase Auth plus `users`, `login_activity`, OTP/reset and `trusted_devices` | Existing individual commit and compatibility behavior remains unchanged | Remaining `security.*` | Planned; requires separate bounded ownership review |
...
Phase 1B-1 does not change a domain owner. Phase 1B-2A attaches the existing cursor-level security writer only to the four password-login/logout outcomes above; every other runtime attachment remains a future bounded change.
...
4. **Phase 1B-2A — Password login/logout security events (Implemented in repository; not applied or deployed)** — exactly four definitions are repository-integrated with caller-owned terminal transactions and a forward activation migration. Shared TEST remains at the Phase 1B-1 all-false projection.
5. **Phase 1B-2 remaining runtime event integration (Planned)** — attach canonical writers only through separately reviewed domain-owner changes.
...
The canonical catalog therefore contains 149 Planned-lifecycle events plus 8 Deferred QR-payment events. Deferred events are reported separately and excluded from the Planned total. Exactly four Planned-lifecycle security definitions are integrated in the repository by Phase 1B-2A; the other 145 Planned definitions and all 8 Deferred definitions remain unintegrated.
