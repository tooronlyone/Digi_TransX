...
| Latest capability-status reconciliation | 2026-08-14; Phase 1A through Phase 1B-2C3B1, its bounded security corrections, and genuine-session-activity runtime are implemented in main and authorized shared TEST. Phase 1B-2C3B2 MPIN/access-lock frontend UX and its bounded responsive/navigation corrections are implemented in repository main. The frontend-only integration required and performed no shared-TEST/database change; no production access, rollout, public launch, or deployment occurred. |
...
| Database rollout/inspection mode | Only immutable migrations `180000`, `190000`, and `200000` were applied; pre/post reconciliation used read-only transactions and verification probes ended with `ROLLBACK` |
| Scheduler state during inspection | Disabled with `DIGITRANSX_ENABLE_SCHEDULER=0`; no scheduler process active |
| Phase | Phase 0 registry; Phase 1A through Phase 1B-2C3B1 implemented in main/shared TEST; Phase 1B-2C3B2 frontend and responsive/navigation corrections implemented in repository main without a shared-TEST/database change; production remains undeployed and the product remains under development/private testing |
...
| Trusted device / fast login | Phase 1B-2C3B1 implemented in main and authorized shared TEST | [`trusted_device_service.py`](../backend/auth/trusted_device_service.py) is the single transaction-participating owner for secure generation, SHA-256 digestion, current-digest-only resolution, creation, locked rotation, and revocation. The `dtx_device_token` cookie is HttpOnly, SameSite=Lax, path `/`, bounded to at most 30 days, and Secure outside explicit local development. Legacy MPIN authentication routes fail closed and clear the device cookie. | Hardened digest-only `trusted_devices`; invalidated legacy `users.mpin_hash`/`users.mpin_enabled`; zero-row `user_sessions` keeps the stable `(trusted_device_id,user_id)` binding | Previous digests and legacy MPIN credentials do not authenticate; device/session management UI remains Planned for 2C6. |
| Secure MPIN / software access lock | Phase 1B-2C3B1 backend implemented in main and shared TEST; Phase 1B-2C3B2 frontend in main; Phase 1B-2C3C1 step-up foundation on its feature branch only; not deployed | [`mpin_service.py`](../backend/auth/mpin_service.py) owns exact MPIN validation, memory-hard verification, the shared atomic failure counter, and fifth-failure lockout. [`step_up_service.py`](../backend/auth/step_up_service.py) recognizes exactly six approved action descriptors and owns digest-only three-minute issuance plus verify/claim/consume/reconciliation primitives. | A generation-bearing `mpin_credentials` row, digest-only software proof in `user_sessions`, and the zero-fixture `mpin_step_up_authorizations` ledger. The separate step-up proof binds the exact user/session/device/generation/action/resource/request and is returned raw once with `no-store`. | Eligible roles remain exactly `logistics_provider`, `service_seeker`, and `everyday_user`. The six Category A mutations and their UI remain deliberately unconnected. No shared-TEST or production deployment occurred. |
...
- The legacy MPIN authentication bypass remains disabled. Secure enrollment/unlock/password-unlock/change/disable/reset and software access lock remain implemented in main/shared TEST. Phase 1B-2C3C1 adds the action-bound issuance/ledger foundation on its feature branch, reuses the same failure counter for issuance/change/disable, and leaves all six Category A mutations unconnected. `.expired_inactivity`, logout-all, mass revocation, the device/session dashboard, GPS, and Email/SMS OTP remain Planned.
...
Shared TEST remains at empty event tables, 170 catalog rows, 162 Planned, 8 Deferred, 156 writable, 21 integrated, 141 Planned-unintegrated, 135 writable-unintegrated, six Operations, and signature `b57a59369062e678a7b269cd61d4e01e`. The un-deployed Phase 1B-2C3C1 branch target is 172 rows, 164 Planned, 8 Deferred, 158 writable, 23 integrated, 141 Planned-unintegrated, 135 writable-unintegrated, six Operations, and signature `82bd918f68377090324c3a15da210769`; migration verification retains empty event tables.

Phase 1B-2C3C1 corrects the two original step-up contracts and integrates exactly `.step_up_succeeded` and `.step_up_failed`, because the issuance route emits both. It adds `.step_up_consumed` and `.step_up_reconciliation_required` as Planned/writable but unintegrated definitions; the writer guard continues to reject both until a reviewed domain owner emits them. `.expired_inactivity` also remains rejected.
...
Phase 1B-2C3A integration commit `3771b5e21f02dab1048e90e14131a00db90baf08` and migration `20260801170000_durable_session_runtime_events.sql` remain implemented in main and authorized shared TEST. The applied `20260801180000_secure_mpin_access_lock.sql` has immutable SHA-256 `33365c7486301dee906c4171c823e13e457660f67576762df980484e2c24d006`; it changed the event signature from `87c1377e1404933c69b1a90ac9962937` to `8dcc0c1ecaf1df3fdad9d0be30f6be03` and created no MPIN/session/event fixture. Applied `20260801190000_reset_authorization_claims.sql` has immutable SHA-256 `b3f2b18270588e8046651fb1776b1704938a292e3c3ca105334cd0be38ed690e` and activates no event. Applied `20260801200000_genuine_session_activity.sql` has immutable SHA-256 `0f1f0be3d18c8693e396c3b699fc8442dd8361f2c976b30656a6e03a3c852e71`; it activates only `security.session.refreshed`, changed the signature to `b57a59369062e678a7b269cd61d4e01e`, and created no fixture. Logout-all, password/reset/account-block mass revocation, expiry-event processing, the device/session management dashboard, high-risk step-up, GPS, and OTP security flows remain Planned.

The un-applied Phase 1B-2C3C1 migration `20260801210000_mpin_step_up_authorization_foundation.sql` has repository SHA-256 `736b38ea1844b6f0c9a7087a7ff2441a1b7214b08d968352d52e147d68ee7dc5`; it is guarded by event signature `82bd918f68377090324c3a15da210769` and two accepted physical foundation signatures arising from fresh versus historical additive column layout. Disposable PostgreSQL verification proved convergence, exact valid-state reapplication, abort on injected ledger-column corruption, and zero fixtures. Shared TEST remains unchanged.

Post-rollout reconciliation found zero `mpin_credentials`, `user_sessions`, reset authorizations, and event rows; no Auth or business fixture and no replacement MPIN credential were created. All 47 pre-existing public-table row counts and all checked financial aggregates were unchanged, and legacy MPIN active/hash state remained zero. `mpin_credentials` has RLS enabled; `service_role` has exactly `SELECT`, `INSERT`, `UPDATE`, and `DELETE`, while `PUBLIC`, `anon`, and `authenticated` have no prohibited access. Reset authorization storage grants its service owner exactly `SELECT`, `INSERT`, and `UPDATE`, its sequence exactly `SELECT` and `USAGE`, and rollback-only probes persisted no rows.
...
- [`backend/events/catalog.py`](../backend/events/catalog.py) remains the only machine-readable catalog owner. Main and shared TEST have 21 integrated Planned security definitions: the preceding twelve plus access-lock, seven non-step-up MPIN integrations, and exactly `security.session.refreshed`.
...
Final verification passed 74/74 frontend Node/static tests, 53/53 focused PostgreSQL MPIN/signup-security tests, changed-file ESLint, a 754-module production build, and the responsive containment matrix at 320, 360, 375, 390, 414, 768, 1024, and 1440 pixels plus 667×320 landscape. The manual real-Chrome responsive/native-keyboard gate passed. High-risk financial-action MPIN step-up, logout-all, remaining GPS/security flows, mandatory OTP flows, the device/session security dashboard, later operational/security integrations, and production deployment remain Planned or otherwise unimplemented as classified elsewhere. The product remains under development/private testing.

## 7. Security and login locked contract

Everything in this section is **Planned** unless explicitly cross-referenced as current.

Phase 1B-2A is the main/shared-TEST cross-reference for `security.login.started`, `security.login.failed`, `security.login.succeeded`, and `security.logout.completed`. Phase 1B-2C3B1 integrates `security.session.access_locked`, the seven non-step-up MPIN names, and `security.session.refreshed` in main and shared TEST. Lifecycle classification remains Planned; `.expired_inactivity`, both MPIN step-up names, and every other name not explicitly integrated remain rejected.
...
| Canonical event foundation | `backend/events` catalog/contract/writers | Shared TEST `security_events`, `business_audit_events`, and `canonical_event_catalog_projection` | Shared TEST is at the applied empty-table, 170-definition, 21-integration state with narrow ACL and signature `b57a59369062e678a7b269cd61d4e01e` | Access-lock, seven non-step-up MPIN events, and `.refreshed` are integrated; `.expired_inactivity` and the two MPIN step-up events remain rejected | Continue only through separately reviewed domain-owner changes using the existing writer/caller transaction | Any direct route table write, client identity/time, second catalog, partial activation, or guard regression |
...
| MPIN/password/reset/device | Auth routes plus canonical MPIN/session/trusted-device services | Main/shared TEST `mpin_credentials`, access-proof fields in `user_sessions`, hardened reset claims, Supabase password re-verification, and hardened `trusted_devices` | Legacy fast-login authentication and legacy verifier activation remain disabled; the forward migration cleared legacy verifier state without creating replacement credentials; shared TEST retains zero credentials, sessions, and reset authorizations | Access-lock, seven non-step-up MPIN events, and trusted genuine-session refresh are integrated in main/shared TEST; the shared eligible-role MPIN/access-lock frontend is implemented in repository main | Action-bound step-up, password/reset/block mass revocation, logout-all, expiry-event processing, the device/session security dashboard, GPS, and OTP delivery security remain Planned |
...
6. **Phase 1B-2C0 — device/session/MPIN event contracts (Implemented in main and authorized shared TEST; not deployed)** — migration `20260801140000_device_session_mpin_event_contracts.sql` added twelve contracts and formalized five existing contracts. At Phase 1B-2C0 completion all seventeen were unintegrated; later phases integrated trusted-device, session, secure-MPIN/access-lock, and genuine-activity definitions. High-risk payment/action MPIN step-up, expiry-event processing, GPS, Email OTP, and SMS OTP security flows remain Planned, and no payment or wallet behavior changed.
7. **Phase 1B-2C1 — durable server-session foundation (Implemented in main and authorized shared TEST; not deployed)** — one zero-row `user_sessions` table and the canonical session service established the digest-only token model, genuine activity, lock, expiry, rotation, and revocation state. At 2C1 completion the service was intentionally unused; Phase 1B-2C3A made it the sole runtime authentication authority and Phase 1B-2C3B1 added MPIN/access-lock and genuine-activity behavior. The foundation's RLS and narrow service privileges remain unchanged. Step-up, logout-all/remote and password/account-block mass revocation, expiry-event processing, the device/session management dashboard, GPS, and Email/SMS OTP security flows remain Planned.
8. **Phase 1B-2C2 — trusted-device hardening (Implemented in main and authorized shared TEST; not deployed)** — integration commit `eb10d8eff93499a67e424bea5a60fd0245d46c4f` and migration `20260801160000_trusted_device_hardening.sql` (SHA-256 `016c99fbf924e6eb578f563f6611a757fe1228293e8bc31f40e1282d63753609`) migrated 31 eligible rows in place to exact 32-byte SHA-256 digests while preserving stable IDs, user ownership, and same-user session binding. It removed raw-token storage, added 30-day maximum absolute expiry plus revocation/locked-rotation state, centralized runtime ownership, integrated exactly the three trusted-device events, and disabled the legacy MPIN authentication bypass while retaining then-legacy MPIN data. Later phases activated durable-session authority, invalidated legacy MPIN verifier state, and implemented secure MPIN/genuine activity. Step-up and action/resource-bound grants, logout-all/remote management, password/reset/block mass revocation, expiry-event processing, the security dashboard, GPS, and Email/SMS OTP security flows remain Planned.
...
- 14 logical data objects in main/shared TEST: two event tables, `user_sessions`, and zero-row `mpin_credentials` implemented; 10 remain Not implemented.

The catalog contains 162 Planned-lifecycle events plus 8 Deferred QR-payment events (170 combined), of which 156 are writable. Main and authorized shared TEST have exactly 21 integrated Planned security definitions, 141 Planned-unintegrated definitions, and 135 writable-unintegrated definitions. The six Operations definitions remain non-writable and unintegrated. The current semantic signature is `b57a59369062e678a7b269cd61d4e01e`; the pre-rollout shared-TEST signature was `87c1377e1404933c69b1a90ac9962937`. No production deployment occurred.
