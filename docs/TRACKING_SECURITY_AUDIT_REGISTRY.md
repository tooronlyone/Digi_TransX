...
| Latest capability-status reconciliation | 2026-08-08; Phase 1A through Phase 1B-2C1 are implemented in main and authorized shared TEST. Phase 1B-2C1 adds the unused durable server-session database/service foundation only; it is not wired into Flask authentication or deployed. |
...
| Phase | Phase 0 registry; Phase 1A through Phase 1B-2C1 implemented in main and authorized shared TEST; all durable-session runtime integration remains Planned |
...
- Migration/fresh schema produced zero event rows. In main and shared TEST, the integrated guard admits only the seven integrated definitions and rejects all 149 writable-but-unintegrated Planned definitions, including the seventeen Phase 1B-2C0 contracts, signup GPS, and OTP, plus Operations and Deferred QR definitions; it preserves existing envelope, actor, idempotency, append-only, and ACL guarantees.
...
Phase 1B-1 does not change a domain owner. Phase 1B-2A attaches the existing cursor-level security writer to the four password-login/logout outcomes. Phase 1B-2B attaches it to the three signup outcomes in main and shared TEST; GPS, OTP, and every other runtime attachment remain future bounded changes.
