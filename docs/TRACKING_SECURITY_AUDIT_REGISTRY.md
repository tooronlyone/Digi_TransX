...
| Latest capability-status reconciliation | 2026-08-09; Phase 1A through Phase 1B-2C2 are implemented in main and authorized shared TEST. Phase 1B-2C2 was integrated at `eb10d8eff93499a67e424bea5a60fd0245d46c4f`; it is not deployed to production. Durable server-session runtime remains unwired and undeployed. |
...
| Phase | Phase 0 registry; Phase 1A through Phase 1B-2C2 implemented in main and authorized shared TEST; durable-session runtime integration remains Planned |
...
| Password login | Phase 1B-2A bounded evidence implemented in main and shared TEST; broader auth remains unstandardized | [`login()`](../backend/auth/routes.py) durably emits server-owned `security.login.started` before provider verification, then keeps terminal `login_activity`, successful `last_login_at`, trusted-device persistence, and the terminal event in one separate caller transaction. The Flask session is created only after that transaction commits. | Main: `security_events`, `login_activity`, `users.last_login_at`, hardened `trusted_devices`, Flask cookie; shared TEST has ten integrated definitions and the integrated-state guard | Blocked, unknown, and wrong-password attempts have one indistinguishable public 401 response while internal canonical result codes remain coarse and distinct. No GPS, mandatory Email OTP, risk engine, device/session limits, or high-risk restrictions. |
| Logout | Phase 1B-2A bounded evidence implemented in main and shared TEST | [`logout()`](../backend/auth/routes.py) validates authenticated CSRF, derives the actor before clearing, attempts `security.logout.completed`, and always clears the local session if audit persistence fails. | Main: `security_events` plus client cookie; shared TEST has ten integrated definitions and the integrated-state guard | No server session exists to revoke; the current trusted-device credential is not revoked by ordinary logout. Anonymous/invalid-CSRF requests emit no logout event. |
