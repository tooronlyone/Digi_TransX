...
- Migration/fresh schema produced zero event rows. The integrated guard changes the write contract from all 143 catalog-writable definitions to only the four integrated definitions; its direct-SQL matrix rejects the 139 writable-but-unintegrated Planned definitions, Operations, and Deferred QR events while preserving existing envelope, actor, idempotency, append-only, and ACL guarantees.
...
| Canonical event foundation | `backend/events` catalog/contract/writers | Shared TEST `security_events`, `business_audit_events`, and `canonical_event_catalog_projection` | Phase 1B-1 plus Phase 1B-2A activation/guard applied with empty tables, 157 definitions and narrow ACL | Foundation plus exactly four integrated auth definitions | Continue only through separately reviewed domain-owner changes using the existing writer/caller transaction | Any direct route table write, client identity/time, second catalog, partial activation, or guard regression |
