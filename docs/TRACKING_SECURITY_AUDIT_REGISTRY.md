...
| Latest capability-status reconciliation | 2026-08-03; Phase 1B-2A remains applied to authorized shared TEST and merged to main. Phase 1B-2B is implemented only on `feature/phase-1b2b-signup-events`; it activates three bounded signup events in the repository but has not been merged or applied to shared TEST. |
...
| Phase | Phase 0 registry; Phase 1A containment, Phase 1B-1, and Phase 1B-2A implemented in main and authorized shared TEST; Phase 1B-2B signup integration implemented only on its feature branch; later runtime integrations remain Planned |
...
| --- | --- | --- | --- | --- |
| Signup | Phase 1B-2B implemented on feature branch only | [`signup()`](../backend/auth/routes.py) commits anonymous `security.signup.started` before external Auth creation. It then commits public user/profile, successful `login_activity`, trusted device, and `security.signup.completed` together; Flask session, CSRF, and device cookie follow only after commit. Failure records one anonymous coarse `security.signup.failed` code and uses bounded Auth compensation/reconciliation. | Feature branch: `security_events`, `auth.users`, `users`, one role-profile table, `login_activity`, `trusted_devices`; shared TEST remains unchanged at four integrations | Exactly `security.signup.started`, `.failed`, and `.completed` are feature-branch integrated. GPS and Email/SMS OTP remain Planned; provider messages, identity fields, device token, and form values are excluded from canonical evidence. |
| Password login | Phase 1B-2A bounded evidence implemented in main; broader auth remains unstandardized | [`login()`](../backend/auth/routes.py) durably emits server-owned `security.login.started` before provider verification, then keeps terminal `login_activity`, successful `last_login_at`, trusted-device persistence, and the terminal event in one separate caller transaction. The Flask session is created only after that transaction commits. | Main: `security_events`, `login_activity`, `users.last_login_at`, `trusted_devices`, Flask cookie; shared TEST has four integrated rows and the integrated-state guard | Blocked, unknown, and wrong-password attempts have one indistinguishable public 401 response while internal canonical result codes remain coarse and distinct. No GPS, mandatory Email OTP, risk engine, device/session limits, or high-risk restrictions. |
| Logout | Phase 1B-2A bounded evidence implemented in main | [`logout()`](../backend/auth/routes.py) validates authenticated CSRF, derives the actor before clearing, attempts `security.logout.completed`, and always clears the local session if audit persistence fails. | Main: `security_events` plus client cookie; shared TEST has four integrated rows and the integrated-state guard | No server session exists to revoke; current device token remains trusted. Anonymous/invalid-CSRF requests emit no logout event. |
...
- Phase 1B-1 envelope schemas, canonical catalog/writers, explicit environment separation, RLS, idempotency, contract tests, and bounded `service_role` ACL hardening are **Implemented in main and shared TEST**. Phase 1B-2A activates exactly four password-login/logout definitions in main and shared TEST; the guard permits only integrated+writable definitions. Phase 1B-2B on `feature/phase-1b2b-signup-events` activates exactly the three bounded signup definitions through `20260801130000_security_signup_event_integration.sql`; it has not changed shared TEST.
...
- Seventeen forward migrations exist under [`supabase/migrations`](../supabase/migrations). The Phase 1B-1 foundation/ACL, Phase 1B-2A activation, and forward integrated-guard correction are applied to shared TEST and merged to main; the Phase 1B-2B signup integration migration exists only on its feature branch.
...
Shared TEST contains empty `security_events` and `business_audit_events` tables plus the 157-row `canonical_event_catalog_projection`; exactly four login/logout definitions are `integrated=true`, with 149 Planned and 8 Deferred definitions and exactly 143 catalog-writable definitions. `event_outbox` remains absent. `service_role` has exactly `DELETE`, `INSERT`, and `SELECT` on each event table and zero projection access; `anon`, `authenticated`, and `PUBLIC` have zero event or projection access. The applied guard signature is `7b8157021244549cfed79416b40ab662`: only integrated+writable definitions are insertable, all 139 writable-but-unintegrated definitions are rejected, and rolled-back verification left both event tables empty.
...
- [`backend/events/catalog.py`](../backend/events/catalog.py) remains the only machine-readable catalog owner: 150 Planned-lifecycle names plus 8 Deferred QR names. On `feature/phase-1b2b-signup-events`, exactly seven Planned auth definitions are integrated: the four Phase 1B-2A login/logout names plus `security.signup.started`, `.failed`, and `.completed`. Signup failure permits only one coarse `result_code` from `validation_failed`, `account_conflict`, `provider_unavailable`, `persistence_failed`, or `reconciliation_required`; signup-start is anonymous and carries no request data. The other 143 Planned definitions, including signup GPS and OTP, and all 8 Deferred definitions remain unintegrated. Deferred QR and the six Operations names remain non-writable.
...
- Migration/fresh schema produced zero event rows. On this feature branch, the integrated guard admits only the seven integrated definitions and rejects all 137 writable-but-unintegrated Planned definitions, including signup GPS and OTP, plus Operations and Deferred QR definitions; it preserves existing envelope, actor, idempotency, append-only, and ACL guarantees.
- Fresh, partial and exact-reapply migration tests prove fail-closed preservation for wrong column names/types/nullability/defaults, weakened CHECKs, missing indexes/triggers/policies, wrong RLS/grants and drifted helper definitions. Current correction-branch verification adds the integrated-only matrix and passes the complete PostgreSQL backend suite with 657 tests and zero skips; its explicit no-DB run passes 297 while safely skipping 360 PostgreSQL-dependent tests.
...
Phase 1B-2A is implemented in main and authorized shared TEST. Exactly `security.login.started`, `security.login.failed`, `security.login.succeeded`, and `security.logout.completed` are integrated there. Phase 1B-2B is feature-branch-only and adds `security.signup.started`, `security.signup.failed`, and `security.signup.completed` through one forward migration; shared TEST is not changed. The feature branch therefore has seven integrated Planned definitions, while 143 other Planned definitions and all 8 Deferred QR definitions remain unintegrated.
...
- Focused correction validation passes 49 integrated-guard/foundation tests and 35 auth/ACL/baseline tests; the complete backend PostgreSQL suite passes 657 with zero failures/skips. The explicit no-DB run passes 297 and skips 360 database-dependent tests. The production build passes; this frontend package has no test script and no frontend file changed, so changed-file ESLint is not applicable.
- Signup runtime integration remains Planned: this correction defines only the canonical terminal failure vocabulary and does not emit it. MPIN, trusted-device, password/reset, OTP, GPS, risk, session lifecycle, account lock/unlock, business, analytics, and Operations integrations remain Planned. Existing transitional `login_activity`, MPIN, trusted-device, CSRF, role, and password decisions are not replaced.
...
| --- | --- | --- | --- | --- | --- | --- |
| Auth event evidence | Password-login/logout routes plus transitional helper | Shared TEST: `login_activity` 44 rows and empty `security_events`; activation and guard applied | Phase 1B-2A integrates exactly four canonical events while retaining existing activity evidence; only integrated+writable names are insertable | Short committed login-start transaction, followed by one terminal transaction owning activity, success timestamp, trusted device where applicable, and terminal event; canonical payload excludes legacy identifier/IP/device-token fields | Any additional auth event, raw failure/identity field, duplicate start, or split terminal commit |
...
| Canonical event foundation | `backend/events` catalog/contract/writers | Shared TEST `security_events`, `business_audit_events`, and `canonical_event_catalog_projection` | Shared TEST remains at the applied empty-table, 158-definition, four-integration state with narrow ACL; this feature branch activates three additional signup definitions only locally | Foundation plus seven integrated auth definitions on the feature branch | Continue only through separately reviewed domain-owner changes using the existing writer/caller transaction | Any direct route table write, client identity/time, second catalog, partial activation, or guard regression |
...
| Signup | Auth route on feature branch | Supabase Auth plus `users`, one role profile, `login_activity`, `trusted_devices`, and `security_events` | Start commits before external Auth. Public user/profile, activity, trusted device, and completed event share one public transaction; session follows commit. Proven Auth compensation is bounded to the request-owned identity. | `security.signup.started`, `.failed`, `.completed` | **Feature-branch-only; not applied to shared TEST** |
| MPIN/password/reset/device | Existing auth/profile routes and helpers | Supabase Auth plus OTP/reset and `trusted_devices` | Existing compatibility behavior remains unchanged | Remaining `security.*` | Planned; requires separate bounded ownership review |
...
Phase 1B-1 does not change a domain owner. Phase 1B-2A attaches the existing cursor-level security writer to the four password-login/logout outcomes. Phase 1B-2B attaches it only to the three signup outcomes on its feature branch; GPS, OTP, and every other runtime attachment remain future bounded changes.
...
3. **Phase 1B-1 — Canonical event foundation and ACL hardening (Implemented in repository and shared TEST)** — both Phase 1B-1 migrations are applied; the two event tables are present and empty, the 157-definition catalog projection is present, and the narrow ACL/signature is verified.
...
5. **Phase 1B-2B — bounded signup events (Implemented on feature branch only; not merged or applied to shared TEST)** — exactly three signup definitions are integrated through a forward migration and a compensation-aware external-Auth saga; GPS and Email/SMS OTP remain Planned.
6. **Phase 1B-2 remaining runtime event integration (Planned)** — attach canonical writers only through separately reviewed domain-owner changes.
6. **Security** — observation first; risk shadow mode and alerts/cases; later Email OTP after provider/domain readiness. Device/session and raw-token migration is a later dedicated security workstream and must preserve or deliberately migrate MPIN compatibility.
7. **One-Time Business Audit** — attach same-transaction events to canonical One-Time services and prove idempotency/lock behavior.
8. **General Analytics** — replace broad tracker with consent-aware, allowlisted, idempotent analytics.
9. **Operational Monitoring** — structured errors/latency/health/provider/scheduler/release telemetry and incident lifecycle.
10. **Dashboards and retention** — user/admin security surfaces, KPI/funnel views, deletion/aggregation jobs, access audits.
...
On `feature/phase-1b2b-signup-events`, the canonical catalog contains 150 Planned-lifecycle events plus 8 Deferred QR-payment events (158 combined). Deferred events are reported separately and excluded from the Planned total. Exactly seven Planned auth definitions are integrated: four Phase 1B-2A login/logout names and three Phase 1B-2B signup names. The other 143 Planned definitions, including signup GPS and Email/SMS OTP, and all 8 Deferred definitions remain unintegrated. Phase 1B-2B is branch-only and is not applied to shared TEST.
