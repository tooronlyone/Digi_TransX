...
| Latest capability-status reconciliation | 2026-08-12; Phase 1A through Phase 1B-2C3B1, its bounded security corrections, and genuine-session-activity runtime are implemented in main and authorized shared TEST. No production rollout or deployment occurred. |
...
| Phase | Phase 0 registry; Phase 1A through Phase 1B-2C3B1 implemented in main/shared TEST; production remains undeployed |
...
- The legacy MPIN authentication bypass remains disabled. Secure enrollment/unlock/password-unlock/change/disable/reset, the separate access proof, `security.session.access_locked`, permanent fifth-failure MPIN lockout, and trusted visible-interaction `.refreshed` activity are implemented in main and authorized shared TEST. Action-bound MPIN step-up, `.expired_inactivity`, logout-all, password/reset/block mass revocation, frontend MPIN/session UI, GPS, and Email/SMS OTP security flows remain Planned.
