...
| Password login | Phase 1B-2A bounded evidence implemented in repository; broader auth remains unstandardized | [`login()`](../backend/auth/routes.py) emits server-owned `security.login.started` plus one terminal `failed`/`succeeded` event, keeps terminal `login_activity` and `last_login_at` in the same caller transaction, and creates the Flask session only after mandatory success evidence commits. | Repository: `security_events`, `login_activity`, `users.last_login_at`, Flask cookie; shared TEST activation not applied | No GPS, mandatory Email OTP, risk engine, device/session limits, or high-risk restrictions. |
...
- Valid password login writes `started` before credential verification. Terminal `login_activity`, `last_login_at` on genuine success, and the terminal canonical event share one route-owned PostgreSQL transaction. Mandatory success-evidence failure rolls back all three and prevents Flask session issuance; failed authentication remains denied if evidence persistence fails.
...
| --- | --- | --- | --- | --- | --- |
| Password login/logout | Auth route; no event aliases | Supabase Auth verification plus `users.last_login_at`, `login_activity`, `security_events`, trusted-device/session response | Login terminal activity/event and genuine-success timestamp use one caller-owned DB transaction; Supabase verification remains external; session follows committed success evidence. Logout audit owns one DB transaction and session clear is fail-safe. | Exactly four Phase 1B-2A `security.login.*`/`security.logout.completed` events | **Repository-integrated, TEST-unapplied**; no cross-system atomicity is claimed for Supabase Auth verification |
