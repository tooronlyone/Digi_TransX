...
| Latest capability-status reconciliation | 2026-08-10; Phase 1A through Phase 1B-2C3A remain implemented in main and authorized shared TEST. Phase 1B-2C3B1 plus the bounded genuine-session-activity correction are verified only on `feature/phase-1b2c3b1-secure-mpin-backend`; they are not merged, applied to shared TEST, or deployed. |
...
| Phase | Phase 0 registry; Phase 1A through Phase 1B-2C3A implemented in main/shared TEST; Phase 1B-2C3B1 verified on its feature branch only |
...
| Password login | Phase 1B-2C3A durable runtime implemented in main and shared TEST; feature-branch transaction correction verified | [`login()`](../backend/auth/routes.py) independently commits `security.login.started`, performs remote password verification with no PostgreSQL transaction or pooled connection held, then locks and revalidates the current user before atomically persisting successful activity, applicable `last_login_at`, trusted-device mutation and event, one durable session, terminal login evidence, and `security.session.issued`. The committed transaction-owned user snapshot builds the response; cookies and CSRF state issue only after commit. | Main and shared TEST: `security_events`, `login_activity`, `users`, hardened `trusted_devices`, `user_sessions`; shared TEST has 12 integrations | The provider-boundary and post-commit-read corrections are verified only on this feature branch until merged. GPS, mandatory Email OTP, risk engine, session limits, and high-risk restrictions remain Planned. |
...
| Password reset | Partially implemented; feature-branch replay correction verified | Forgot-password sends a six-digit Email OTP and stores only its password hash. The feature branch consumes the latest challenge once under a row lock, creates exactly one signed high-entropy reset authorization in the same transaction, stores only its SHA-256 digest, and atomically claims it before provider I/O. Provider failure/finalization ambiguity leaves a non-replayable explicit reconciliation state. | `password_reset_otps`, hardened `reset_tokens` claim lifecycle | OTP expiry remains 10 minutes, not the locked five; no 60-second resend control or HMAC/device-bound challenge; reset does not revoke sessions/devices. These broader OTP/revocation requirements remain Planned. |
| Password change | Partially implemented; one-shot correction verified on the feature branch | [`request_password_change_otp()`](../backend/profile/routes.py#L59) and [`change_password()`](../backend/profile/routes.py#L95) reuse the row-locked one-shot OTP consumer; wrong or replayed values cannot invoke the Auth provider again. | `password_reset_otps` | No session revocation or security-event contract; delivery infrastructure and the future device-bound OTP contract remain Planned. |
...
| Secure MPIN / software access lock | Phase 1B-2C3B1 verified on the feature branch only | [`mpin_service.py`](../backend/auth/mpin_service.py) owns exact four-ASCII-digit validation, unique 32-byte salt, mandatory environment pepper, versioned `hashlib.scrypt` verification, atomic attempt state, and permanent fifth-failure lockout. [`login_required()`](../backend/auth/helpers.py) centrally distinguishes invalid authentication from a valid locked session and permits only status, unlock, password unlock, and logout while locked. | One user-owned `mpin_credentials` row; digest-only session-bound `dtx_access_proof` state in `user_sessions`; raw proof only in memory and an HttpOnly non-persistent cookie | Eligible roles are only logistics provider, service seeker, and everyday user. Frontend MPIN UI, high-risk step-up, logout-all, dashboard, GPS, and OTP remain Planned. Shared TEST remains at 2C3A until separately authorized. |
| Genuine session inactivity | Verified on the feature branch only | [`GenuineActivity.jsx`](../frontend-react/src/components/GenuineActivity.jsx) accepts only visible, trusted pointer/touch/keyboard events and sends an empty same-origin CSRF-backed signal. [`record_genuine_activity()`](../backend/auth/session_service.py) revalidates and locks the exact session, user, and same-user device, requires the access proof, and advances only genuine activity/inactivity timestamps at most once per 12 hours; [`genuine_session_activity()`](../backend/auth/routes.py) writes `security.session.refreshed` in the same transaction. | Exact `user_sessions` row plus one digest-referenced canonical event per durable refresh | Passive page views, `/auth/me`, polling, background tabs, timers, failed requests, and synthetic events do not refresh. Seven continuous inactive days require full login; the 30-day absolute boundary never moves. `security.session.expired_inactivity` remains Planned because no global expiry-event owner is implemented. |
| Flask session | Phase 1B-2C3A non-authoritative compatibility state implemented in main; genuine-activity correction verified on feature branch | [`build_auth_success_response()`](../backend/auth/helpers.py) retains only bounded CSRF and presentation activity state. [`login_required()`](../backend/auth/helpers.py) requires the durable-session and exact trusted-device cookies, then derives `request.current_user`, role, and profile relationships from PostgreSQL. | `user_sessions` is authoritative; Flask state contains no authoritative user ID, role, profile identity, authentication flag, or durable token | Forged signed identity cannot authenticate. Passive `/auth/me` and Flask presentation timestamps do not update durable genuine activity. |
...
- On the feature branch, Flask signing configuration fails startup on a missing, short, or known-placeholder secret outside explicit `local`/`test` mode. Unidentified, staging, production, and unknown deployment modes force Secure Flask/authentication/trusted-device/access-proof cookies; explicit `local` additionally requires a loopback host. Deprecated `FLASK_ENV` cannot downgrade this boundary.
- The legacy MPIN authentication bypass remains disabled. On the 2C3B1 feature branch only, secure enrollment/unlock/password-unlock/change/disable/reset, the separate access proof, `security.session.access_locked`, and permanent fifth-failure MPIN lockout are implemented. The follow-up genuine-activity correction integrates `.refreshed` with trusted visible interaction and seven-day rolling inactivity enforcement. Action-bound MPIN step-up, `.expired_inactivity`, logout-all, password/reset/block mass revocation, frontend MPIN/session UI, GPS, and Email/SMS OTP remain Planned.
...
| Documents | Private download authorization corrected on the feature branch; broader evidence remains partial | Chat media requires an exact message-to-thread mapping plus participant or assigned group-admin authorization. Sensitive truck documents require the owning user or platform admin; deliberately inline vehicle images remain authenticated catalog media. New storage keys use opaque random names, chat provider upload runs outside PostgreSQL before row-locked terminal revalidation, and provider errors are sanitized. | [`backend/shared/storage.py`](../backend/shared/storage.py), `chat_messages`, `chat_threads`, `documents`, `vehicles`, private `shipment-documents` bucket | Access evidence/events, private orphan reconciliation, and a broader document-classification lifecycle remain incomplete; URL possession alone is not authorization. |
...
Shared TEST remains unchanged at the 2C3A state: empty `security_events` and `business_audit_events`, 170 catalog rows, 162 Planned, 8 Deferred QR, 156 writable, 12 integrated, and signature `87c1377e1404933c69b1a90ac9962937`. The corrected feature-branch projection has the same 170/162/8/156 lifecycle totals but exactly 21 integrations, 141 Planned-unintegrated, 135 writable-unintegrated, six Operations, and signature `b57a59369062e678a7b269cd61d4e01e`. It adds the 2C3B1 access-lock/seven MPIN integrations and exactly `security.session.refreshed`; `event_outbox` remains absent, and ACL/append-only guard behavior is unchanged.

The corrected feature-branch projection is locally verified with 170 rows: 162 Planned, 8 Deferred QR, 156 writable, 21 integrated definitions, 141 Planned-unintegrated definitions, 135 writable-unintegrated definitions, and six Operations definitions. Its semantic signature is `b57a59369062e678a7b269cd61d4e01e`. Main and shared TEST remain at the preceding 12-integration 2C3A projection.

Phase 1B-2C0 added `security.session.issued`, `security.session.access_locked`, `security.trusted_device.rotated`, `security.mpin.enrolled`, `security.mpin.changed`, `security.mpin.disabled`, `security.mpin.unlock_succeeded`, `security.mpin.unlock_failed`, `security.mpin.locked`, `security.mpin.reset_completed`, `security.mpin.step_up_succeeded`, and `security.mpin.step_up_failed`; it formalized contracts for the existing `security.session.refreshed`, `security.session.expired_inactivity`, `security.session.revoked`, `security.trusted_device.added`, and `security.trusted_device.removed`. Phase 1B-2C2 integrated the three trusted-device names, Phase 1B-2C3A integrated `.issued`/`.revoked`, 2C3B1 integrated `.access_locked` plus the seven non-step-up MPIN names, and the genuine-activity correction integrates exactly `.refreshed`. `.expired_inactivity` and both MPIN step-up events remain rejected.
...
Phase 1B-2C3A integration commit `3771b5e21f02dab1048e90e14131a00db90baf08` and migration `20260801170000_durable_session_runtime_events.sql` remain implemented in main and authorized shared TEST. The feature-only `20260801180000_secure_mpin_access_lock.sql` has SHA-256 `33365c7486301dee906c4171c823e13e457660f67576762df980484e2c24d006`; it changes the event signature from `87c1377e1404933c69b1a90ac9962937` to `8dcc0c1ecaf1df3fdad9d0be30f6be03`, creates no MPIN/session/event fixture, and is not applied to shared TEST. The follow-up `20260801200000_genuine_session_activity.sql` has SHA-256 `0f1f0be3d18c8693e396c3b699fc8442dd8361f2c976b30656a6e03a3c852e71`, activates only `security.session.refreshed`, changes the signature to `b57a59369062e678a7b269cd61d4e01e`, creates no fixture, and is not applied to shared TEST. Logout-all, password/reset/account-block mass revocation, expiry-event processing, frontend session-management UI/dashboard, high-risk step-up, GPS, and OTP remain Planned.
...
| `20260801180000_secure_mpin_access_lock.sql` | Adds the canonical MPIN credential table, three access-proof/password-verification session fields, strict RLS/ACL/constraints, invalidates legacy verifier state, and activates exactly eight events | Verified only on the 2C3B1 feature branch; SHA-256 `33365c7486301dee906c4171c823e13e457660f67576762df980484e2c24d006`; signature `87c1377e1404933c69b1a90ac9962937` to `8dcc0c1ecaf1df3fdad9d0be30f6be03`; sequential/fresh/reapply/corrupt paths create zero credential/session/event fixtures; not applied to shared TEST |
| `20260801190000_reset_authorization_claims.sql` | Forward-only reset-authorization claim lifecycle, legacy authorization invalidation, strict state constraints, digest uniqueness, and service-role-only ACL/RLS | Verified only on the feature branch through sequential/fresh/reapply/partial/corrupt paths; it activates no canonical event and is not applied to shared TEST. |
| `20260801200000_genuine_session_activity.sql` | Activates exactly `security.session.refreshed` for the transaction-owned genuine-activity runtime; no table change or fixtures | Verified only on the feature branch through sequential/fresh/reapply/partial/corrupt paths; SHA-256 `0f1f0be3d18c8693e396c3b699fc8442dd8361f2c976b30656a6e03a3c852e71`; signature `8dcc0c1ecaf1df3fdad9d0be30f6be03` to `b57a59369062e678a7b269cd61d4e01e`; not applied to shared TEST. |
...
- [`backend/events/catalog.py`](../backend/events/catalog.py) remains the only machine-readable catalog owner. The 2C3B1 feature branch has 21 integrated Planned security definitions; main/shared TEST remain at 12 until separately authorized. The feature branch adds access-lock plus seven non-step-up MPIN integrations, and the bounded genuine-activity correction adds exactly `security.session.refreshed`.
...
- Migration/fresh schema produces zero event rows. On the 2C3B1 feature branch, the integrated guard admits 21 definitions and rejects the remaining 135 writable-but-unintegrated Planned definitions; main/shared TEST retain the preceding 12/144 state.
...
- Phase 1B-2C3B1 feature verification passed 746 PostgreSQL-backed tests with zero failures/skips and explicit disconnected mode passed 332 with 414 expected database skips. All 14 frontend Node/static tests, test-source ESLint, production build, Python compileall, migration immutability, fresh/sequential/exact-reapply/corrupt convergence, duplicate, sensitive-log, legacy-SQL-dialect, catalog/registry, and ACL checks passed. Repository-wide frontend lint still has its unchanged pre-existing baseline outside this backend-only phase. No shared TEST write or production access occurred.
...
| Sessions | Canonical [`auth/session_service.py`](../backend/auth/session_service.py); Flask cookie is presentation/CSRF-only | Main/shared TEST durable `user_sessions`; feature branch adds proof digest/expiry and recent password-verification time | 2C3A durable session plus exact trusted device is authoritative. The 2C3B1 feature branch separately evaluates the access proof, atomically transitions to 423 software lock once, and rotates the proof without refreshing authentication or genuine activity. The bounded genuine-activity correction advances only the current session's inactivity boundary and integrates `security.session.refreshed` | Canonical foundation, proof owner, and activity owner are reused; raw session/proof values remain browser-memory cookies only | Keep logout-all, expiry-event processing, dashboard, GPS, and OTP separate | Any raw token persistence, proof in browser storage/JSON, expiry extension, or access-lock bypass |
...
| Canonical event foundation | `backend/events` catalog/contract/writers | Shared TEST `security_events`, `business_audit_events`, and `canonical_event_catalog_projection` | Shared TEST is at the applied empty-table, 170-definition, twelve-integration 2C3A state with narrow ACL | The 2C3B1 feature branch integrates exactly nine more definitions: access-lock, seven non-step-up MPIN events, and `.refreshed`; `.expired_inactivity` and the two MPIN step-up events remain rejected | Continue only through separately reviewed domain-owner changes using the existing writer/caller transaction | Any direct route table write, client identity/time, second catalog, partial activation, or guard regression |
...
| MPIN/password/reset/device | Auth routes plus canonical MPIN/session/trusted-device services | Feature branch `mpin_credentials`, access-proof fields in `user_sessions`, Supabase password re-verification, and hardened `trusted_devices` | Legacy fast-login authentication and legacy verifier activation remain disabled; the forward migration clears legacy verifier state without creating replacement credentials | Feature branch integrates access-lock, seven non-step-up MPIN events, and trusted genuine-session refresh; shared TEST still has only the trusted-device/session integrations through 2C3A | Frontend MPIN UI, action-bound step-up, password/reset/block mass revocation, logout-all, expiry-event processing, dashboard, GPS, and OTP remain Planned |
...
6. **Phase 1B-2C0 — device/session/MPIN event contracts (Implemented in main and authorized shared TEST; not deployed)** — migration `20260801140000_device_session_mpin_event_contracts.sql` added twelve contracts and formalized five existing contracts. At Phase 1B-2C0 completion all seventeen were unintegrated; Phase 1B-2C2 later integrated the three trusted-device definitions and Phase 1B-2C3A later integrated session issued/revoked. The current feature branch later implements software MPIN unlock and seven-day genuine-inactivity enforcement; high-risk payment/action MPIN step-up, expiry-event processing, GPS, Email OTP, and SMS OTP remain Planned, and no payment or wallet behavior changed.
7. **Phase 1B-2C1 — durable server-session foundation (Implemented in main and authorized shared TEST; not deployed)** — one zero-row `user_sessions` table and the canonical session service established the digest-only token model, genuine activity, lock, expiry, rotation, and revocation state. At 2C1 completion the service was intentionally unused; Phase 1B-2C3A now makes it the sole runtime authentication authority. The foundation added one table, four indexes, nine table constraints, and one service-role policy; its RLS and narrow service privileges remain unchanged. The current feature branch later implements MPIN enrollment/unlock/lockout/reset, automatic access locking, and seven-day genuine-inactivity enforcement; step-up, logout-all/remote and password/account-block mass revocation, expiry-event processing, management UI/dashboard, GPS, and Email/SMS OTP remain Planned.
8. **Phase 1B-2C2 — trusted-device hardening (Implemented in main and authorized shared TEST; not deployed)** — integration commit `eb10d8eff93499a67e424bea5a60fd0245d46c4f` and migration `20260801160000_trusted_device_hardening.sql` (SHA-256 `016c99fbf924e6eb578f563f6611a757fe1228293e8bc31f40e1282d63753609`) migrated 31 eligible rows in place to exact 32-byte SHA-256 digests while preserving stable IDs, user ownership, and same-user session binding. It removed raw-token storage, added 30-day maximum absolute expiry plus revocation/locked-rotation state, centralized runtime ownership, integrated exactly the three trusted-device events, and disabled the legacy MPIN authentication bypass while retaining MPIN data. Phase 1B-2C3A later activated durable-session issuance/enforcement and removed Flask signed-cookie authentication authority. The current feature branch later implements secure MPIN enrollment/unlock/attempt limits/lockout/reset and seven-day genuine inactivity; step-up and action/resource-bound grants, logout-all/remote management, password/reset/block mass revocation, expiry-event processing, the security dashboard, GPS, and Email/SMS OTP remain Planned.
9. **Phase 1B-2C3A — durable server-session runtime authority (Implemented in main and authorized shared TEST; not deployed)** — integration commit `3771b5e21f02dab1048e90e14131a00db90baf08` makes `public.user_sessions` the sole runtime authentication authority through the canonical session service. Password signup/login issue one digest-only session bound to the exact active same-user trusted device in the terminal caller-owned transaction; all protected routes resolve current identity/role/profile relationships from the durable row and current database state. Flask signed state is bounded CSRF/presentation-only. Current logout revokes exactly its locked session and leaves all other sessions untouched. Migration `20260801170000_durable_session_runtime_events.sql` (SHA-256 `fe4d8440ad683396b31da7ec8da0e335ceaa8936c1fb1c4d5f51a24058f57aeb`) integrates exactly `security.session.issued` and `.revoked`, advancing main/shared TEST to 12 integrations and signature `87c1377e1404933c69b1a90ac9962937`. The current feature branch later implements MPIN 2C3B1 and genuine-session activity; logout-all/revocation triggers, expiry-event processing, dashboard, GPS, and OTP remain Planned.
10. **Phase 1B-2C3B1 — secure MPIN and software access lock (Verified on feature branch only; not merged/applied/deployed)** — one canonical user-owned MPIN row uses exact four-ASCII-digit validation, unique random salt, mandatory no-default server pepper, versioned memory-hard scrypt, atomic failures, and permanent fifth-failure lockout. A separate at-least-256-bit proof is stored raw only in an HttpOnly browser-session cookie and as a SHA-256 digest with an at-most-eight-hour server lifetime. Login/signup issue it only after terminal commit; unlock rotation extends neither session/device expiry nor genuine activity. Password unlock creates no session. Legacy verifier state is cleared and never activated. Deterministic mutation order is session → user → trusted device → MPIN credential → canonical event. The bounded genuine-activity correction adds trusted session refresh and seven-day rolling inactivity enforcement. The UI, high-risk step-up, logout-all, expiry-event processing, dashboard, GPS, and OTP remain Planned.
11. **Phase 1B-2 remaining runtime event integration (Planned)** — attach canonical writers only through separately reviewed domain-owner changes.
12. **Security** — observation first; risk shadow mode and alerts/cases; later Email OTP after provider/domain readiness. Durable current-session runtime is implemented in main; secure MPIN/runtime access locking and trusted genuine-session activity are verified on the 2C3B1 feature branch. Logout-all/security-triggered mass revocation, expiry-event processing, step-up, and device/session management remain later dedicated security workstreams.
...
16. **Dashboards and retention** — user/admin security surfaces, KPI/funnel views, deletion/aggregation jobs, access audits.
...
- 13 logical data objects in main/shared TEST: two event tables plus `user_sessions` implemented and 10 still Not implemented. The 2C3B1 feature branch adds the fourteenth logical object, zero-fixture `mpin_credentials`; shared TEST remains unchanged.

The catalog contains 162 Planned-lifecycle events plus 8 Deferred QR-payment events (170 combined), of which 156 are writable. The corrected feature branch has exactly 21 integrated Planned security definitions, 141 Planned-unintegrated definitions, and 135 writable-unintegrated definitions. The six Operations definitions remain non-writable and unintegrated. The feature-branch semantic signature is `b57a59369062e678a7b269cd61d4e01e`; main and authorized shared TEST remain unchanged at 12 integrations and `87c1377e1404933c69b1a90ac9962937`.
