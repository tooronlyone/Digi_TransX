...
| Latest capability-status reconciliation | 2026-08-21; Phase 1A through Phase 1B-2C3C1 are implemented in main and authorized shared TEST. Phase 1B-2C3C1 adds only the MPIN step-up authorization foundation; its six Category A domain mutations and frontend `StepUpProvider` remain pending. No production access, rollout, public launch, or deployment occurred. |
...
| Phase | Phase 0 registry; Phase 1A through Phase 1B-2C3C1 implemented in main/shared TEST; six Category A mutations and frontend step-up integration remain pending; production remains undeployed and the product remains under development/private testing |
...
Phase 1B-2C3C1 corrects the two original step-up contracts and integrates exactly `.step_up_succeeded` and `.step_up_failed`, because the issuance route emits both. It adds `.step_up_consumed` and `.step_up_reconciliation_required` as Planned/writable but unintegrated definitions; shared-TEST rollback probes and the writer guard reject both and every other unintegrated definition until a reviewed domain owner emits them. `.expired_inactivity` also remains rejected.
...
Phase 1B-2A is the main/shared-TEST cross-reference for `security.login.started`, `security.login.failed`, `security.login.succeeded`, and `security.logout.completed`. Phase 1B-2C3B1 integrates `security.session.access_locked`, the seven non-step-up MPIN names, and `security.session.refreshed`; Phase 1B-2C3C1 integrates only `security.mpin.step_up_succeeded` and `.failed`. Lifecycle classification remains Planned; `.step_up_consumed`, `.step_up_reconciliation_required`, `.expired_inactivity`, and every other name not explicitly integrated remain rejected.
