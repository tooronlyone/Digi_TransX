...
- Migration/fresh schema produces zero event rows. On the 2C3B1 feature branch, the integrated guard admits 20 definitions and rejects the remaining 136 writable-but-unintegrated Planned definitions; main/shared TEST retain the preceding 12/144 state.
...
| Sessions | Canonical [`auth/session_service.py`](../backend/auth/session_service.py); Flask cookie is presentation/CSRF-only | Main/shared TEST durable `user_sessions`; feature branch adds proof digest/expiry and recent password-verification time | 2C3A durable session plus exact trusted device is authoritative. The 2C3B1 feature branch separately evaluates the access proof, atomically transitions to 423 software lock once, and rotates the proof without refreshing authentication or genuine activity | Canonical foundation and proof owner are reused; raw session/proof values remain browser-memory cookies only | Keep logout-all, inactivity processing, dashboard, GPS, and OTP separate | Any raw token persistence, proof in browser storage/JSON, expiry extension, or access-lock bypass |
...
| Canonical event foundation | `backend/events` catalog/contract/writers | Shared TEST `security_events`, `business_audit_events`, and `canonical_event_catalog_projection` | Shared TEST is at the applied empty-table, 170-definition, twelve-integration 2C3A state with narrow ACL | The 2C3B1 feature branch integrates exactly eight more definitions; `.refreshed`, `.expired_inactivity`, and the two MPIN step-up events remain rejected | Continue only through separately reviewed domain-owner changes using the existing writer/caller transaction | Any direct route table write, client identity/time, second catalog, partial activation, or guard regression |
...
| MPIN/password/reset/device | Auth routes plus canonical MPIN/session/trusted-device services | Feature branch `mpin_credentials`, access-proof fields in `user_sessions`, Supabase password re-verification, and hardened `trusted_devices` | Legacy fast-login authentication and legacy verifier activation remain disabled; the forward migration clears legacy verifier state without creating replacement credentials | Feature branch integrates access-lock and seven non-step-up MPIN events; shared TEST still has only the trusted-device/session integrations through 2C3A | Frontend MPIN UI, action-bound step-up, password/reset/block mass revocation, logout-all, seven-day inactivity, dashboard, GPS, and OTP remain Planned |
