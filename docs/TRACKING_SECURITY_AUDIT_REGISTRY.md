...
13. **Communication-channel policy correction (Implemented on `fix/remove-sms-channel-policy`; not merged, applied, or deployed)** — the obsolete settings API preference and transporter SMS control are removed, legacy SMS preference keys are omitted from API projections and later settings saves, and no SMS provider/configuration/dependency/runtime owner remains. No SMS event definition or database projection row existed, so the exact removed-event set is empty, no `220000` migration is created, and catalog 172/164/8/158/23/141/135/6 plus signature `82bd918f68377090324c3a15da210769` remain unchanged. Legitimate phone/CNIC/contact data remains unchanged. Existing password-reset/password-change Email OTP is distinguished from Planned verified-email signup, untrusted-device login, locked-MPIN recovery, and external notification delivery.
