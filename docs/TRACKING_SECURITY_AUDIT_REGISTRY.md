...
| Latest capability-status reconciliation | 2026-08-10; Phase 1A through Phase 1B-2C3A remain implemented in main and authorized shared TEST. Phase 1B-2C3B1 is verified only on `feature/phase-1b2c3b1-secure-mpin-backend`; it is not merged, applied to shared TEST, or deployed. |
...
- The legacy MPIN authentication bypass remains disabled. On the 2C3B1 feature branch only, secure enrollment/unlock/password-unlock/change/disable/reset, the separate access proof, `security.session.access_locked`, and permanent fifth-failure MPIN lockout are implemented. Action-bound MPIN step-up, `.refreshed`, `.expired_inactivity`, logout-all, password/reset/block mass revocation, seven-day genuine-inactivity enforcement, frontend MPIN/session UI, GPS, and Email/SMS OTP remain Planned.
...
Shared TEST remains unchanged at the 2C3A state: empty `security_events` and `business_audit_events`, 170 catalog rows, 162 Planned, 8 Deferred QR, 156 writable, 12 integrated, and signature `87c1377e1404933c69b1a90ac9962937`. The 2C3B1 feature branch projection has the same 170/162/8/156 lifecycle totals but exactly 20 integrations, 142 Planned-unintegrated, 136 writable-unintegrated, six Operations, and signature `8dcc0c1ecaf1df3fdad9d0be30f6be03`. It activates only `security.session.access_locked` and the seven non-step-up MPIN events. `event_outbox` remains absent; ACL and append-only guard behavior are unchanged.

The Phase 1B-2C3B1 feature-branch projection is locally verified with 170 rows: 162 Planned, 8 Deferred QR, 156 writable, 20 integrated definitions, 142 Planned-unintegrated definitions, 136 writable-unintegrated definitions, and six Operations definitions. Its semantic signature is `8dcc0c1ecaf1df3fdad9d0be30f6be03`. Main and shared TEST remain at the preceding 12-integration 2C3A projection.

Phase 1B-2C0 added `security.session.issued`, `security.session.access_locked`, `security.trusted_device.rotated`, `security.mpin.enrolled`, `security.mpin.changed`, `security.mpin.disabled`, `security.mpin.unlock_succeeded`, `security.mpin.unlock_failed`, `security.mpin.locked`, `security.mpin.reset_completed`, `security.mpin.step_up_succeeded`, and `security.mpin.step_up_failed`; it formalized contracts for the existing `security.session.refreshed`, `security.session.expired_inactivity`, `security.session.revoked`, `security.trusted_device.added`, and `security.trusted_device.removed`. Phase 1B-2C2 integrated the three trusted-device names, Phase 1B-2C3A integrated `.issued`/`.revoked`, and the 2C3B1 feature branch integrates `.access_locked` plus the seven non-step-up MPIN names. `.refreshed`, `.expired_inactivity`, and both MPIN step-up events remain rejected.
...
The catalog contains 162 Planned-lifecycle events plus 8 Deferred QR-payment events (170 combined), of which 156 are writable. The 2C3B1 feature branch has exactly 20 integrated Planned security definitions, 142 Planned-unintegrated definitions, and 136 writable-unintegrated definitions. The six Operations definitions remain non-writable and unintegrated. The feature-branch semantic signature is `8dcc0c1ecaf1df3fdad9d0be30f6be03`; main and authorized shared TEST remain unchanged at 12 integrations and `87c1377e1404933c69b1a90ac9962937`.
