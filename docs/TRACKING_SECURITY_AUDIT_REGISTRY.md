...
| Latest capability-status reconciliation | 2026-08-01; Phase 1B-2A bounded auth integration implemented in the repository but not applied to shared TEST or deployed |
...
| Phase | Phase 0 registry; Phase 1A containment implemented; schema-trigger/RLS baseline and Phase 1B-1 foundation/ACL implemented in repository and shared TEST; Phase 1B-2A login/logout integration implemented in repository only and not yet applied or deployed; every other runtime integration remains Planned |
...
| Password login | Phase 1B-2A bounded evidence implemented in repository; broader auth remains unstandardized | [`login()`](../backend/auth/routes.py) durably emits server-owned `security.login.started` before provider verification, then keeps terminal `login_activity`, successful `last_login_at`, trusted-device persistence, and the terminal event in one separate caller transaction. The Flask session is created only after that transaction commits. | Repository: `security_events`, `login_activity`, `users.last_login_at`, `trusted_devices`, Flask cookie; shared TEST activation not applied | Blocked, unknown, and wrong-password attempts have one indistinguishable public 401 response while internal canonical result codes remain coarse and distinct. No GPS, mandatory Email OTP, risk engine, device/session limits, or high-risk restrictions. |
| Logout | Phase 1B-2A bounded evidence implemented in repository | [`logout()`](../backend/auth/routes.py) validates authenticated CSRF, derives the actor before clearing, attempts `security.logout.completed`, and always clears the local session if audit persistence fails. | Repository: `security_events` plus client cookie; shared TEST activation not applied | No server session exists to revoke; current device token remains trusted. Anonymous/invalid-CSRF requests emit no logout event. |
...
- Phase 1B-1 envelope schemas, canonical catalog/writers, explicit environment separation, RLS, idempotency, contract tests, and bounded `service_role` ACL hardening are **Implemented in the repository and shared TEST**. Phase 1B-2A integrates exactly four password-login/logout definitions in the repository only; the other 145 Planned-lifecycle definitions and all 8 Deferred definitions remain unintegrated.
...
- Fourteen forward migrations exist under [`supabase/migrations`](../supabase/migrations). The Phase 1B-1 foundation and ACL correction are applied to shared TEST; the Phase 1B-2A activation migration is repository-only and unapplied.
...
Shared TEST contains empty `security_events` and `business_audit_events` tables plus the 157-row `canonical_event_catalog_projection`; all definitions remain `integrated=false`, with 149 Planned and 8 Deferred definitions and exactly 143 database-writable definitions. `event_outbox` remains absent. After the applied ACL correction, `service_role` has exactly `DELETE`, `INSERT`, and `SELECT` on each event table and zero projection access; `anon`, `authenticated`, and `PUBLIC` have zero event or projection access. The shared-TEST signature remains `772212260b85fd6b5cd4aa35ca9ffdfb`. Phase 1B-2A is not applied or deployed there and no shared-TEST event row was created.
...
- Migration/fresh schema produced zero event rows. Valid security/business writers, all 143 currently writable definitions, wrong name/table/category/version/retention rejection, Operations/Deferred/non-writable rejection, sensitive/oversized/nested/malformed payload rejection, server-owned actor fields, replay/conflict/concurrency, atomic rollback, UPDATE rejection, real-role RLS/grants, and retention-compatible DELETE were proven.
- Fresh, partial and exact-reapply migration tests prove fail-closed preservation for wrong column names/types/nullability/defaults, weakened CHECKs, missing indexes/triggers/policies, wrong RLS/grants and drifted helper definitions. The ACL correction adds 8 focused tests; the combined Phase 1A/baseline/foundation/ACL regression passes 106; the complete PostgreSQL backend suite passes 630 with zero skips; and the explicit no-DB run passes 289 while safely skipping 341 PostgreSQL-dependent tests.
