...
create unique index trusted_devices_id_user_unique on trusted_devices (id, user_id);
...
create table user_sessions (
    ...
    foreign key (trusted_device_id, user_id)
        references trusted_devices (id, user_id)
);
...
create unique index uq_trips_id_order
    on shipment_trips (id, order_id);
create unique index uq_trips_id_transporter
    on shipment_trips (id, transporter_user_id);
create unique index uq_shipments_id_client
    on shipments (id, client_user_id);
...
-- Minimal one-time chat thread (checkout opens exactly one per accepted trip).
create table chat_threads (
    id              bigint generated by default as identity primary key,
    client_user_id  bigint,
    transporter_user_id bigint,
    agreement_post_id bigint,
    is_group_chat   boolean not null default false,
    admin_user_id   bigint,
    dispute_trip_id bigint,
    shipment_id     bigint,
    one_time_trip_id bigint,
    last_message_at timestamptz,
    created_at      timestamptz not null default now()
);
create unique index uniq_chat_thread_one_time
    on chat_threads (shipment_id) where shipment_id is not null;
...
create table chat_messages (
    ...
    content         text,
    media_path      text,
    media_document_id bigint,
    media_request_status text,
    is_read         boolean not null default false,
    created_at      timestamptz not null default now()
);

-- One-time delivery dispute (queried by the order-detail route).
create table shipment_disputes (
    id              bigint generated by default as identity primary key,
...
create unique index uniq_open_dispute_per_trip
    on shipment_disputes (trip_id) where status = 'open';
...
create table shipment_trip_reviews (
    ...
    constraint shipment_trip_reviews_shipment_client_fk
        ...
        references shipments (id, client_user_id)
        on delete cascade,
    constraint shipment_trip_reviews_trip_shipment_fk
        foreign key (trip_id, shipment_id)
        references shipment_trips (id, order_id)
        on delete cascade,
    constraint shipment_trip_reviews_trip_transporter_fk
        foreign key (trip_id, transporter_user_id)
        references shipment_trips (id, transporter_user_id)
        on delete cascade
);

-- In-app lifecycle notifications (user references are plain columns here).
...
create unique index uniq_notification_event
    on shipment_notifications (trip_id, user_id, notification_type)
...
create or replace function test_record_shipment_status_history()
returns trigger language plpgsql as $$
begin
...
create trigger trg_shipments_status_history
    ...
    for each row execute function test_record_shipment_status_history();
"""
...
def db(pg_connection):
    ...
    with pg_connection.cursor() as cur:
        cur.execute(
            ...
            "shipment_notifications, shipment_status_history, shipment_disputes, "
            "shipment_trip_reviews, "
            "chat_messages, chat_threads, documents, payments, "
            ...
            "vehicles, user_action_logs, password_reset_otps, users "
...
def app_env(seeded_db, pg_session_info, monkeypatch):
    ...
    app.register_blueprint(trucks_routes.trucks_blueprint)
...
def client(app_env):
    ...
    def login(user):
        ...
        db.execute(
            ...
            "role=excluded.role, legacy_role=excluded.legacy_role",
            (
                user["id"],
                user.get("full_name", "Fixture User"),
                user.get("email", ""),
                user.get("phone", ""),
                user.get("cnic", ""),
                user.get("role", "customer"),
                user.get("role", "service_seeker"),
            ),
        )
        ...
        device = db.execute(
            ...
            (hashlib.sha256(device_token.encode()).digest(), user["id"]),
        ).fetchone()
        db.execute(
