...
def test_wrong_password_and_unknown_account_have_same_response_and_private_events(
    ...
    events = _event_rows(url)
    assert [event["event_name"] for event in events].count("security.login.started") == 2
    failed = [event for event in events if event["event_name"] == "security.login.failed"]
    assert len(failed) == 2
    assert {event["metadata"]["result_code"] for event in failed} == {"invalid_credentials"}
    for event in failed:
    ...
    assert all(row["status"] == "failed" for row in _rows(url, "login_activity"))
    assert _rows(url, "users")[0]["last_login_at"] is None


def test_blocked_and_provider_failure_use_only_coarse_codes(auth_client, monkeypatch):
    client, url = auth_client
