...
def normalize_truck_model(truck_company, truck_model):
    company = re.sub(r"\s+", " ", truck_company or "").strip()
    model = re.sub(r"\s+", " ", truck_model or "").strip()
    if not model:
        return None
    if not company:
        return model

    company_prefix = re.compile(rf"^{re.escape(company)}(?=$|[\s\-_/]+|\d)", re.IGNORECASE)
    match = company_prefix.match(model)
    if match:
        suffix = model[match.end():].strip(" -_/")
...
def get_catalog_fields(type_key):
    ...
    for key, label in (
        ...
        ("payload_min_tons", "Weight min (tons)"),
        ("payload_max_tons", "Weight max (tons)"),
...
def build_truck_payload(row):
    ...
    return {
        ...
        "payload_min_tons": row["payload_min_tons"],
        "payload_max_tons": row["payload_max_tons"],
        "volume_min_cbm": row["volume_min_cbm"],
        "volume_max_cbm": row["volume_max_cbm"],
        "bed_length_ft": row["bed_length_ft"],
        "bed_width_ft": row["bed_width_ft"],
...
def build_configuration_payload(row):
    ...
    return {
        ...
        "bed_length_ft": truck["bed_length_ft"] or "",
        "bed_width_ft": truck["bed_width_ft"] or "",
        "bed_height_ft": truck["bed_height_ft"] or "",
